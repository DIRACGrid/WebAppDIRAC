...
class TokenManagerHandler(WebHandler):
  ...
  def web_getTokenManagerData(self):
    user = self.getUserName()
    if user.lower() == "anonymous":
      return {"success": "false", "error": "You are not authorize to access these data"}

    result = self.tm.getUsersTokensInfo(list(json.loads(self.get_argument("username", "[]"))))
    gLogger.info("*!*!*!  RESULT: \n%s" % result)
    if not result['OK']:
    ...
    for record in result['Value']:
      tokens.append({'tokenid': record['user_id'],
