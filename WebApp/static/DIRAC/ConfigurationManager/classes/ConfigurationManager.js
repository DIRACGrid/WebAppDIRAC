Ext.define("DIRAC.ConfigurationManager.classes.ConfigurationManager", {
  ...
  __sendSocketMessage: function(oData) {
    ...
    if (!me.isConnectionEstablished) {
      var sMessage =
        "There is no connection established with the server.\nDo you want to reconnect now?";
    ...
    socket.onmessage = function(e) {
      ...
      if (parseInt(oResponse.success, 10) == 0) {
        ...
        switch (oResponse.op) {
          case "init":
            me.setNodeText(
              me.treeStore.getRootNode(),
              oResponse.name + " [" + oResponse.version + "]"
            );
          ...
          case "setOptionValue":
            ...
            me.txtOptionValuePanelTextArea.setValue(
              me.__stringToList(oNode.get("csValue")).join("\n")
            );
            me.setNodeText(
              oNode,
              oNode.get("csName") + " = " + oNode.get("csValue")
            );
          ...
          case "setComment":
            ...
            me.valuePanel.csComment = me
              .__commentToList(oNode.get("csComment"))
              .join("\n");
          ...
          case "commitConfiguration":
            GLOBAL.APP.CF.alert(
              "The changes in the configuration have been successfuly commited !",
              "info"
            );
            me.btnCommitConfiguration.show();
            me.__setChangeMade(false);
          ...
          case "rollback":
            Ext.dirac.system_info.msg(
              "Notification",
              "The version roll back to " + oResponse.version
            );
            break;
          case "download":
            try {
          ...
          case "showCommitDiff":
            var cb = function(window) {
              if (
                confirm(
                  "Do you want to apply the configuration changes you've done till now?"
                )
              ) {
                me.treePanel.body.mask(
  ...
  buildUI: function() {
    ...
    me.btnResetConfig = new Ext.Button({
      ...
      handler: function() {
        if (me.changeMade) {
          if (
            confirm(
              "If you reload you might loose the changes you've might made.\nDo you want to reload?"
            )
          ) {
            me.__sendSocketMessage({
              op: "resetConfiguration"
    ...
    var bBarElems = [
      me.btnViewConfigAsText,
      me.btnDownloadConfigAsText,
      me.btnResetConfig
    ];
    ...
    if (
      ...
      Ext.Array.indexOf(
        GLOBAL.USER_CREDENTIALS.properties,
        "CSAdministrator"
      ) != -1
    ) {
    ...
    me.txtCommentValuePanelTextArea = new Ext.create(
      "Ext.form.field.TextArea",
      {
        fieldLabel: "Comment",
        labelAlign: "top",
  ...
  __showConfigTextInWindow: function(sTextToShow) {
    ...
    var oWindow = me
      .getContainer()
      .createChildWindow("Configuration As Text", false, 700, 500);
  ...
  __showConfigDiffInWindow: function(oResponse, cbFunction) {
    ...
    var oWindow = me
      .getContainer()
      .createChildWindow(
        "Current Configuration Difference",
        false,
        700,
        iWinHeight
      );
    ...
    oResponse.html = oResponse.html.replace(
      new RegExp("&amp;nbsp;", "g"),
      "&nbsp;"
    );
      ...
      listeners: {
        resize: function(oComp, width, height, oldWidth, oldHeight, eOpts) {
          ...
          for (var i = 0; i < oComp.items.length; i++) {
            ...
            oItem.setPosition(
              0,
              Math.ceil(delta_pos * parseFloat(oItem.lineNumber))
            );
          }
        }
      }
    ...
    var delta_pos =
      (1.0 * (oBlocksPanel.getHeight() - 10)) /
      parseFloat(oResponse.totalLines);
  ...
  __oprSetValuesForValuePanel: function(oModule, oNode) {
    ...
    oModule.valuePanel.csComment = oModule
      .__commentToList(oNode.get("csComment"))
      .join("\n");
    ...
    if (oNode.isLeaf()) {
      oModule.valuePanel.csValue = oModule
        .__stringToList(oNode.get("csValue"))
        .join("\n");
      oModule.txtOptionValuePanelTextArea.setValue(oModule.valuePanel.csValue);
      oModule.txtCommentValuePanelTextArea.setValue(
        oModule.valuePanel.csComment
      );
  ...
  __oprActionValuePanel: function(oButton, eOpts) {
    ...
    if (oNode.isLeaf()) {
      var sNewValue = oModule
        .__stringToList(oModule.txtOptionValuePanelTextArea.getValue(), "\n")
        .join(",");
  ...
  __oprMenuPasteNode: function(oItem, e, eOpts) {
    ...
    if (oModule.copyNode != null) {
      ...
      while (bNameExists) {
        ...
        sNewName = window.prompt(
          "The name already exists. What's the name for the copy?",
          sNewName
        );
  ...
  __oprMenuRenameNode: function(oItem, e, eOpts) {
    var oNode = oItem.parentMenu.node;
    var oModule = oItem.parentMenu.moduleObject;
    var sNewName = window.prompt(
      "What's the new name for " + oNode.get("csName") + " ?"
    );
    if (sNewName == null) return;
  ...
  __oprMenuDeleteNode: function(oItem, e, eOpts) {
    ...
    if (
      !window.confirm(
        "Are you sure you want to delete " + oModule.__getNodePath(oNode) + "?"
      )
    )
      return;
    oModule.__sendSocketMessage({
  ...
  __oprMenuCreateOption: function(oItem, e, eOpts) {
    var oFunc = function(oModule, oNode) {
      var sValue = oModule
        .__stringToList(oModule.txtElementValue.getValue(), "\n")
        .join(",");
