  ...
  tools: [
    {
      ...
      handler: function(event, toolEl, panelHeader) {
        ...
        for (var i = 0; i < widget.tools.length; i++) {
          if (
            widget.tools[i].type == "maximize" ||
            widget.tools[i].type == "close" ||
            widget.tools[i].type == "collapse-right"
          ) {
            widget.tools[i].hide();
  ...
  initComponent: function() {
    ...
    me.timeline = Ext.create("Ext.panel.Panel", {
      ...
      items: [
        {
          html:
            "<div id='" + me.id + "-timeline-plot' style='width:100%;'></div>",
          xtype: "box"
        }
      ]
    });
    ...
    Ext.Ajax.request({
      ...
      params: {
        ...
        element: selection.element
          ? Ext.JSON.encode([selection.element])
          : Ext.JSON.encode(["Resource"])
      },
      scope: me,
    ...
    Ext.Ajax.request({
      ...
      success: function(response) {
        ...
        if (jsonData["success"] == "true") {
          ...
          document.getElementById(me.id + "-timeline-plot").style.height =
            "" + height + "px";

          var width = me.timeline.getWidth() - 20;
          document.getElementById(me.id + "-timeline-plot").style.width =
            "" + width + "px";
          var chart = new google.visualization.AnnotatedTimeLine(
            document.getElementById(me.id + "-timeline-plot")
          );
          ...
          for (var i = 0; i < jsonData["result"].length; i++) {
            ...
            row = [
              Ext.Date.parse(jsonData["result"][i][1], "Y-m-d H:i:s"),
              me.__statusValue(jsonData["result"][i][0]),
              title,
              annotation
            ];
            rows.push(row);
