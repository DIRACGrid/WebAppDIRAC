Ext.define("DIRAC.SystemAdministration.classes.SystemAdministration", {
  ...
  buildUI: function() {
          ...
          if (!bResponseOK) {
            ...
            if ("updateStamp" in oStore) {
              if ("date" in oStore.proxy.reader.rawData)
                oStore.updateStamp.setText(
                  "Updated: " + oStore.proxy.reader.rawData["date"]
                );
    ...
    me.systemInfoGrid = Ext.create("Ext.grid.Panel", {
      ...
      listeners: {
        beforecellcontextmenu: function(
          oTable,
          td,
          cellIndex,
          record,
    ...
    Ext.Ajax.request({
      ...
      success: function(response) {
        ...
        if (response["success"] == "true") {
          ...
          me.__oprPrepareAndShowWindowText(
            response["result"],
            sComponent,
            sSystem,
            sHostName
  ...
  oprGetHostErrors: function() {
    ...
    var sHostName = GLOBAL.APP.CF.getFieldValueFromSelectedRow(
      me.systemInfoGrid,
      "Host"
    );
    me.getContainer().body.mask("Wait ...");
  ...
  __oprPrepareAndShowWindowText: function(
    sTextToShow,
    component,
    system,
    hostname
    ...
    var oWindow = me
      .getContainer()
      .createChildWindow(
        "Log file for: " + component + "/" + system + "@" + hostname,
        false,
        800,
        500
      );
  ...
  __oprPrepareAndShowWindowGrid: function(
    oData,
    sTitle,
    oFields,
    oColumns,
  ...
  __executeAction: function(sAction, sEventSource) {
    ...
    if (sEventSource == 1) {
      sHost = GLOBAL.APP.CF.getFieldValueFromSelectedRow(
        me.systemInfoGrid,
        "Host"
      );
    } else {
  ...
  __executeComponentAction: function(sAction, sEventSource) {
    ...
    if (sEventSource == 1) {
      var sHost = GLOBAL.APP.CF.getFieldValueFromSelectedRow(
        me.hostGrid,
        "Host"
      );
      var sName = GLOBAL.APP.CF.getFieldValueFromSelectedRow(
        me.hostGrid,
        "Name"
      );
      var sSystem = GLOBAL.APP.CF.getFieldValueFromSelectedRow(
        me.hostGrid,
        "System"
      );
  ...
  formSendMail: function(oUsers, oGroups, sMail, iType) {
    ...
    var oSendBtn = new Ext.Button({
      ...
      handler: function() {
        ...
        var sUsers = (oUsers.isInverseSelection()
          ? oUsers.getInverseSelection()
          : oUsers.getValue()
        ).join(",");
        var sGroups = (oGroups.isInverseSelection()
          ? oGroups.getInverseSelection()
          : oGroups.getValue()
        ).join(",");

        if (
          Ext.util.Format.trim(sUsers) == "" &&
          Ext.util.Format.trim(sGroups) == ""
        ) {
  ...
  rendererGridColumn: function(value, record) {
    var me = this;
    var result = "";
    var lastHeartBeat = Ext.Date.parse(
      record.get("LastHeartbeat"),
      "Y-m-d H:i:s"
    );
    var now = new Date();
