Ext.define("DIRAC.JobLaunchpad.classes.JobLaunchpad", {
  ...
  buildUI: function() {
      ...
      handler: function() {
        ...
        for (var sKey in me.textualFields) {
          if (me.textualFields[sKey].mandatory) {
            me.textualFields[sKey].object.setValue(
        ...
        oBottomToolbar.add([
          {
            xtype: "tbtext",
            text:
              "<b style='color:red'>The selected group is not allowed to submit new jobs !</b>"
          }
        ]);
      }
    } else {
    ...
    me.mainFormPanel = new Ext.create("Ext.form.Panel", {
      ...
      items: [
        me.fsetPredefinedSetsSection,
        me.fsetJdlSection,
        me.fsetInputSandboxSection
      ]
    });
      ...
      if (sKey != "InputSandbox") {
        if (sKey in me.textualFields) {
          if (me.textualFields[sKey].object != null) {
            me.textualFields[sKey].object.setValue(
              me.predefinedSets[sPredefinedSet][sKey]
            );
          } else {
            me.__createJdlField(
              sKey,
  ...
  oprAddNewFileField: function() {
    ...
    var sFileFieldName =
      "fileField" + me.fsetInputSandboxSection.items.getCount();
      ...
      listeners: {
        change: function(oComp, sValue, eOpts) {
          ...
          for (var i = 0; i < iLength; i++) {
            var oItem = oComp.moduleObject.fsetInputSandboxSection.getComponent(
              i
            );
          ...
          for (var i = 0; i < iLength; i++) {
            ...
            if (oItem.self.getName() != "Ext.form.field.Text") {
              var iFileSize = oComp.moduleObject.getFileSize(
                oItem.fileInputEl.dom
              );
  ...
  oprAddNewLfnTextField: function(sValue) {
    ...
    var sLfnTextFieldName =
      "lfnField" + me.fsetInputSandboxSection.items.getCount();
  ...
  showProxyStatus: function(sMode) {
    ...
    if (sMode == "true") {
      sBtnText =
        sBtnText + '<span style="color:#009900; font-weight:bold">Valid</span>';
    } else if (sMode == "false") {
      sBtnText =
        sBtnText +
        '<span style="color:#FF0000; font-weight:bold">Not Valid</span>';
    } else if (sMode == "check") {
      sBtnText =
        sBtnText +
        '<span style="color:#FF9900; font-weight:bold">Checking</span>';
    } else {
  ...
  __createJdlField: function(sFieldName, sValue, bSetCheckbox) {
    ...
    me.textualFields[sFieldName].object = new Ext.create(
      "Ext.form.field.Text",
      {
