Ext.define("DIRAC.FileCatalog.classes.FileCatalog", {
        ...
        next: function() {
          ...
          if (!(oBlockData[0] in me.queryData)) {
            ...
            oIter.error_message =
              "'" +
              oBlockData[0] +
              "' was not found in the current set of metadata fields !";
              ...
              for (var i = 0; i < oDataArray.length; i++) {
                if (me.queryData[oBlockData[0]].indexOf(oDataArray[i]) == -1) {
                  ...
                  oIter.error_message =
                    "'" +
                    oDataArray[i] +
                    "' value was not found as a value of the '" +
                    oBlockData[0] +
                    "' field !";
                  GLOBAL.APP.CF.alert(oIter.error_message, "error");
                  me.setLoading(false);
          ...
          switch (oBlockData[1]) {
            case "varchar(128)":
              var oNewBlock = me.__getDropDownField(
                oBlockData[0],
                oBlockData[1]
              );
  ...
  buildUI: function() {
    ...
    me.queryPanelToolbarCenter.add([
      me.btnSubmitLeftPanel,
      me.btnResetLeftPanel,
      me.btnClearQuery
    ]);
      ...
      listeners: {
        load: function(oStore, records, successful, eOpts) {
          if (oStore.proxy.getReader().rawData["total"] == 0)
            GLOBAL.APP.CF.alert(
              "There were no data matching your selection and access policy!",
              "info"
            );

          me.pagingToolbar.updateStamp.setText(
            "Updated: " + oStore.proxy.getReader().rawData["date"]
          );
    ...
    me.pagingToolbar.btnShowQuery = new Ext.Button({
      ...
      handler: function() {
        var oWindow = me
          .getContainer()
          .createChildWindow("Show Query", false, 400, 300);
    ...
    if (
      ...
      var items = GLOBAL.APP.configData.configuration.FileCatalog.pagingToolbar
        .replace(/, /g, ",")
        .split(",");
      for (var i = 0; i < items.length; i++) {
        var element = items[i];
        pagingToolbarItems.push(
          me.pagingToolbar.hasOwnProperty(element)
            ? me.pagingToolbar[element]
            : element
        );
      }
    } else {
    ...
    me.pagingToolbar.toolbar.items.insert(
      4,
      me.pagingToolbar.toolbar.items.items[21]
    );
    me.pagingToolbar.toolbar.items.insert(
      24,
      me.pagingToolbar.toolbar.items.items[7]
    );
    me.pagingToolbar.toolbar.items.insert(
      21,
      me.pagingToolbar.toolbar.items.items[22]
    );
        ...
        itemclick: function(oView, oRecord, item, index, e, eOpts) {
          if (me.__isEveryBlockBlured()) {
            switch (oRecord.get("Type")) {
              ...
              default:
                me.queryPanel.add(
                  me.__getValueField(oRecord.get("Name"), oRecord.get("Type"))
                );
                break;
            }
          } else {
  ...
  __removeLastBlockIfEmpty: function() {
    ...
    if (me.queryPanel.items.length > 0) {
      var oLastBlock = me.queryPanel.items.getAt(
        me.queryPanel.items.length - 1
      );
        ...
        success: function(oReponse) {
          ...
          if (oResponse.success == "true") {
            ...
            for (var i = 0; i < oBackData[oThisBlock.fieldName].length; i++)
              oList.push([
                oBackData[oThisBlock.fieldName][i],
                oBackData[oThisBlock.fieldName][i]
              ]);
    ...
    Ext.Ajax.request({
      ...
      success: function(response) {
        ...
        if (response["success"] == "true") {
          ...
          var sUrl =
            GLOBAL.BASE_URL +
            "FileCatalog/getSelectedFiles?archivePath=" +
  ...
  __getSelectedFiles2Launchpad: function() {
    ...
    Ext.Ajax.request({
      url:
        GLOBAL.BASE_URL +
        "JobLaunchpad/getLaunchpadSetupWithLFNs?path=" +
  ...
  __getValueBlockDescription: function(oBlock) {
    ...
    if (oBlock.blockType == "string") {
      if (oDropDown.getValue().length > 0) {
        var sSign = me.__getSignByIconCls(
          oButton.iconCls,
          oDropDown.isInverseSelection()
        );
