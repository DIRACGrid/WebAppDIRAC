Ext.define("DIRAC.ActivityMonitor.classes.ActivityMonitor", {
  ...
  __buildActivityMonitor: function() {
    ...
    me.activityMonitorDataStore = new Ext.data.JsonStore({
      ...
      listeners: {
        load: function(oStore, records, successful, eOpts) {
          var bResponseOK =
            oStore.proxy.getReader().rawData["success"] == "true";

          if (!bResponseOK) {
            GLOBAL.APP.CF.alert(
              oStore.proxy.getReader().rawData["error"],
              "info"
            );
    ...
    var oDeleteSelectedActivities = new Ext.Button({
      ...
      handler: function() {
        var oElems = Ext.query(
          "#" + me.activityMonitorPanel.id + " input.checkrow"
        );

        var oItems = [];

        for (var i = 0; i < oElems.length; i++)
          if (oElems[i].checked) oItems.push(oElems[i].value);
    ...
    sCheckboxDefinition +=
      "var oElems=Ext.query('#" + sGridId + " input.checkrow');";
    sCheckboxDefinition +=
      "for(var i=0;i<oElems.length;i++)oElems[i].checked = oChecked;";
    ...
    me.htmlDescriptionVariables +=
      "<td style='width:50%;background-color:#eeeeee;font-weight:bold;padding:5px'>";
    ...
    var oVariables = [
      ...
      [
        "$COMPONENTTYPE",
        "Type of component to generate the activity (for example: service, agent, web)"
      ],
    ...
    for (var i = 0; i < oVariables.length; i++) {
      me.htmlDescriptionVariables += "<tr>";
      me.htmlDescriptionVariables +=
        "<td style='width:50%;font-weight:bold;padding:5px'>";
  ...
  __buildPlotManagement: function() {
          ...
          items: [
            "->",
            {
              ...
              handler: function() {
                ...
                if (
                  me.variableFieldCreator.getValue() ||
                  (!me.variableFieldCreator.getValue() &&
                    oSelectedValues.length > 0)
                ) {
                  var sTypeText = me.restrictByFieldCreator
                    .findRecordByValue(sTypeValue)
                    .get("text");
                  ...
                  me.restrictByFieldCreator.store.remove(
                    me.restrictByFieldCreator.findRecordByValue(sTypeValue)
                  );
    ...
    me.plotManagementFieldCreator.add([
      me.restrictByFieldCreator,
      me.valuesFieldCreator,
      ...
      items: [
        {
          ...
          listeners: {
            click: function() {
              ...
              if (
                me.viewDefinitionData[sType].value in
                me.viewDefinitionDataForServer
              ) {
                delete me.viewDefinitionDataForServer[
                  me.viewDefinitionData[sType].value
                ];
              } else {
                Ext.Array.remove(
                  me.viewDefinitionDataForServerVariable,
    ...
    // -------------------------------------------------------------------------------------
    oTopPanel.add([
      me.plotManagementFieldCreator,
      me.plotManagementViewOptions,
      me.plotManagementViewTree
    ]);
  ...
  __buildPlotViewer: function() {
    ...
    me.plotViewerMainPanel.add([
      me.plotViewerListPanel,
      me.plotViewerResultPanel
    ]);
    ...
    var oSubmitBtn = new Ext.Button({
      ...
      handler: function() {
        ...
        if (oTimeSpan.getValue() == -1) {
          if (
            oCalenFrom.getValue() == null ||
            Ext.Date.format(oCalenFrom.getValue(), "Y-m-d") == ""
          ) {
          ...
          if (
            oCalenTo.getValue() == null ||
            Ext.Date.format(oCalenTo.getValue(), "Y-m-d") == ""
          ) {
                ...
                if (plotsList.length) {
                  ...
                  for (var i = 0; i < plotsList.length; i++) {
                    var oNewImage = Ext.create("Ext.Img", {
                      ...
                      src:
                        GLOBAL.BASE_URL +
                        "ActivityMonitor/getPlotImg?file=" +
                        plotsList[i]
                    });
  ...
  generateViewRequest: function() {
    ...
    if (me.viewDefinitionDataForServer.length == 0) {
      GLOBAL.APP.CF.alert(
        "Select at least one non variable restriction",
        "warning"
      );
      return;
    }
      ...
      success: function(response) {
        ...
        if (response.success == "true") {
          ...
          if (plotsList.length) {
            var oWindow = me
              .getContainer()
              .createChildWindow(sViewName, false, 700, 500);
                  ...
                  items: [
                    {
                      ...
                      handler: function() {
                        ...
                        var sViewName = Ext.util.Format.trim(
                          me.txtViewNameViewOptions.getValue()
                        );

                        if (sViewName.length == 0) {
                          GLOBAL.APP.CF.alert(
                            "The name of the view is missing.",
                            "warning"
                          );
                          return;
                        }
  ...
  __buildSystemViewer: function() {
      ...
      listeners: {
        itemclick: function(record, item, index, e, eOpts) {
          ...
          if (item.get("component") != "") {
            ...
            var oNewPlots = me.__buildPlotView(
    ...
    me.systemPlotViewerMainPanel.add([
      me.systemPlotViewerTreePanel,
      me.systemPlotViewerResultPanel
    ]);
