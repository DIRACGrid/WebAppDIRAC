...
Ext.define("Ext.dirac.core.StateManagement", {
          ...
          if (sAppName == "desktop") {
            for (var sStateName in oStates) {
              if (me.TD.oprVerifyDataStructure(oStates[sStateName])) {
                me.cache["application"][sAppName][sStateName] =
                  oStates[sStateName];
              } else {
                var oNewData = me.TD.oprTransformMainViewDataToCurrentVersion(
                  oStates[sStateName]
                );
          ...
          Ext.Ajax.request({
            ...
            success: function(response) {
              if (response.status == 200) {
                ...
                for (var sStateName in oStates) {
                  me.cache["reference"][sAppName][sStateName] =
                    oStates[sStateName];
                }
  ...
  oprSendDataForSave: function(
    ...
    if (oAppObject.getHelpText && oSendData) {
      if (
        oAppObject.up("panel") &&
        oAppObject.up("panel").childWindows &&
        oAppObject.up("panel").childWindows.length > 0
      ) {
  ...
  isAnyActiveState: function(sAppName, sStateName) {
    ...
    for (var i = 0; i < me.activeStates.length; i++) {
      if (
        sStateName == me.activeStates[i][1] &&
        sAppName == me.activeStates[i][0]
      ) {
        oFound = true;
        break;
      }
    ...
    Ext.Ajax.request({
      ...
      success: function(response) {
        if (response.status == 200) {
          GLOBAL.APP.CF.msg(
            "Notification",
            sStateName + " is available to the public!"
          );
        } else {
          GLOBAL.APP.CF.showAjaxErrorMessage(response);
        }
   ...
   *
   */
  oprLoadSharedState: function(
    sLinkDescription,
    cbAfterLoadSharedState,
    stateName
  ) {
    var me = this;
    ...
    if (oDataItems.length != 4) {
      GLOBAL.APP.CF.alert(
        "The 'Load' data you entered is not valid !",
        "warning"
      );
      return;
    }
    ...
    Ext.Ajax.request({
      ...
      success: function(response) {
        if (response.status == 200) {
          Ext.dirac.system_info.msg(
            "Notification",
            "The shared state has been saved successfully !"
          );
    ...
    Ext.Ajax.request({
      ...
      success: function(response) {
        if (response.status == 200) {
          ...
          for (var sStateName in oStates) {
            me.cache["sharedDesktop"][sAppName][sStateName] =
              oStates[sStateName];
            for (var oState in oStates[sStateName]) {
              if (oState != "Metadata") {
                me.cache["application"][sAppName][oState] =
                  oStates[sStateName][oState];
              }
            }
          }
