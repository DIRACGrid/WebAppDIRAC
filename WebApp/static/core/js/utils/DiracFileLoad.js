/***
 * It is used to load a file from the server.
 */
Ext.define('Ext.dirac.utils.DiracFileLoad', {
	...
	loadFile : function(fileList, callback, scope, preserveOrder) {
		...
		/**
		 * Callback function which is called after each file has been loaded. This
		 * calls the callback passed to load once the final file in the fileList has
		 * been loaded
		 */
		var onFileLoaded = function() {
			loadedFiles++;

			// if this was the last file, call the callback, otherwise load the next
			// file
	...
	__buildScriptTag : function(filename, callback) {
		...
		// console.log('Loader.buildScriptTag: filename=[%s], exten=[%s]', filename,
		// exten);
		if (exten == 'js') {
			...
			// IE has a different way of handling <script> loads, so we need to check
			// for it here
