  ...
  listeners: {
    load: function(oStore, records, successful, eOpts) {
      ...
      var bResponseOK =
        ...
        oStore.proxy.getReader().rawData["OK"] == true;
      if (!bResponseOK) {
        ...
        if (
          oStore.proxy.getReader().rawData &&
          me.scope.grid &&
          me.scope.grid.pagingToolbar
        ) {
          var utcTime = null;
          if (oStore.proxy.getReader().rawData["date"]) {
            var newDate = Ext.Date.parse(
              Ext.String.trim(
                oStore.proxy.getReader().rawData["date"].split("[UTC")[0]
              ),
              "Y-m-d H:i"
            );
            if (newDate) {
              var currentTimestamp =
                me.scope.grid.pagingToolbar.updateStamp.updateTimeStamp;
              if (currentTimestamp != null) {
                ...
                var hours = Math.floor(
                  ((newDate - currentTimestamp) % msDay) / msHour
                );
                var minutes = Math.floor(
                  (((newDate - currentTimestamp) % msDay) % msHour) / msMinute
                );

                utcTime =
                  days +
                  " " +
                  GLOBAL.APP.CF.zfill(hours, 2) +
                  ":" +
                  GLOBAL.APP.CF.zfill(minutes, 2);
              }

              me.scope.grid.pagingToolbar.updateStamp.updateTimeStamp = newDate;
            }
            if (utcTime) {
              me.scope.grid.pagingToolbar.updateStamp.setText(
                "Updated: " +
                  oStore.proxy.getReader().rawData["date"] +
                  "(" +
                  utcTime +
                  ")"
              );
    ...
    beforeload: function(oStore, oOperation, eOpts) {
      ...
      if (me.oDiffFields) {
        ...
        if (
          oStore.proxy.getReader().rawData &&
          oStore.proxy.getReader().rawData["total"] > 0
        ) {
          for (var i = 0; i < oStore.proxy.getReader().rawData["total"]; i++) {
            ...
            try {
              ...
              for (var j = 0; j < me.oDiffFields.Fields.length; j++) {
                me.diffValues[record.data[me.oDiffFields.Id]][
                  me.oDiffFields.Fields[j]
                ] = record.data[me.oDiffFields.Fields[j]];
              }
            } catch (e) {}
          }
        }
