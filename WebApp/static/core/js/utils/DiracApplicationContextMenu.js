...
Ext.define("Ext.dirac.utils.DiracApplicationContextMenu", {
  ...
  constructor: function(oConfig) {
    ...
    if (oConfig) {
      ...
      if (
        oConfig.menu &&
        "Visible" in oConfig.menu &&
        oConfig.menu.Visible.length > 0
      ) {
        for (var i = 0; i < oConfig.menu.Visible.length; i++) {
          ...
          if (oConfig.menu.Visible[i].text == "-") {
            ...
            oMenuItem = new Ext.menu.Item({
              ...
              handler: Ext.bind(
                oConfig.menu.Visible[i].handler,
                me.scope,
                oConfig.menu.Visible[i].arguments,
                false
              ),
              scope: me.scope
            });
      ...
      if (
        oConfig.menu &&
        "Protected" in oConfig.menu &&
        oConfig.menu.Protected.length > 0
      ) {
        for (var i = 0; i < oConfig.menu.Protected.length; i++) {
          ...
          var lDisable =
            ...
            (Ext.Array.indexOf(
              ...
              oConfig.menu.Protected[i].property
            ) !=
              -1) ==
              false
              ? true
          ...
          if (oConfig.menu.Protected[i].text == "-") {
            ...
            oMenuItem = new Ext.menu.Item({
              ...
              handler: Ext.bind(
                oConfig.menu.Protected[i].handler,
                me.scope,
                oConfig.menu.Protected[i].arguments,
                false
              ),
              scope: me.scope,
              disabled: lDisable
        ...
        for (var i = 0; i < subMenu.Visible.length; i++) {
          if (subMenu.Visible[i].text == "-") {
            ...
            var oMenuItem = new Ext.menu.Item({
              ...
              handler: Ext.bind(
                subMenu.Visible[i].handler,
                me.scope,
                subMenu.Visible[i].arguments,
                false
              ),
              scope: me.scope
            });
        ...
        for (var i = 0; i < subMenu.Protected.length; i++) {
          ...
          if (subMenu.Protected[i].text == "-") {
            ...
            var oMenuItem = new Ext.menu.Item({
              ...
              handler: Ext.bind(
                subMenu.Protected[i].handler,
                me.scope,
                subMenu.Protected[i].arguments,
                false
              ),
              scope: me.scope,
              disabled: lDisable
