...
Ext.define("Ext.dirac.utils.PlotView", {
      ...
      handler: function() {
        ...
        if (image != null) {
          ...
          Ext.dirac.system_info.msg(
            "Notification",
            "Please select an image what you want to change!"
          );
        }
      },
      scope: me
            ...
            Ext.Ajax.request({
              ...
              success: function(response) {
                ...
                if (response["success"]) {
                  var src =
                    GLOBAL.BASE_URL +
                    me.scope.handlers[me.reportType] +
                    "/getPlotImg?file=" +
                    response["data"];
  ...
  __loadSelectionData: function(oParams) {
    ...
    me.__additionalDataLoad = function() {
      ...
      switch (oParams["_timeSelector"]) {
        ...
        case -2:
          ...
          for (var i = 0; i < oParams["_quarters"].length; i++)
            oNewQuartersArray.push(
              parseInt(oParams["_quarters"][i].replace(" Q", ""))
            );
      ...
      for (var oParam in oParams) {
        ...
        if (!oFound) {
          for (var i = 0; i < me.fsetSpecialConditions.items.length; i++) {
            var oNewUnderlinedName =
              "_" + me.fsetSpecialConditions.items.getAt(i).getName();

            if (oNewUnderlinedName == oParam) {
              me.fsetSpecialConditions.items
                .getAt(i)
                .setInverseSelection(oParams[oParam][0] == 1);
              try {
                me.fsetSpecialConditions.items
                  .getAt(i)
                  .setValue(Ext.JSON.decode(oParams[oParam]));
              } catch (err) {
                me.fsetSpecialConditions.items
                  .getAt(i)
                  .setValue(oParams[oParam].split(","));
              }
    ...
    for (var i = 0; i < me.fsetSpecialConditions.items.length; i++) {
      ...
      if (oCondItem.getValue().length != 0) {
        if (sIntention == "show_plot") {
          param = oCondItem.isInverseSelection()
            ? oCondItem.getInverseSelection()
            : oCondItem.getValue();
          oParams["_" + oCondItem.getName()] = Ext.JSON.encode(param);
        } else if (sIntention == "save_state") {
          oParams["_" + oCondItem.getName()] = [
            oCondItem.isInverseSelection() ? 1 : 0,
            oCondItem.getValue().join(",")
          ];
        }
      }
    }
  ...
  applyDataToSelection: function(oData, sValue, reportType) {
    ...
    var oSelectionOptions =
      me.reportsDesc[me.actualReport][sValue]["selectionConditions"];
  ...
  __validateConditions: function(bWithMessages) {
    ...
    if (
      me.cmbDomain.getValue() == null ||
      Ext.util.Format.trim(me.cmbDomain.getValue()) == ""
    ) {
      if (bWithMessages)
      ...
      me.cmbPlotGenerate.getValue() == null ||
      Ext.util.Format.trim(me.cmbPlotGenerate.getValue()) == ""
    ) {
      if (bWithMessages)
      ...
      me.cmbGroupBy.getValue() == null ||
      Ext.util.Format.trim(me.cmbGroupBy.getValue()) == ""
    ) {
      if (bWithMessages)
    ...
    switch (me.cmbTimeSpan.getValue()) {
      case -1:
        ...
        if (
          me.calendarFrom.getValue() != null &&
          me.calendarTo.getValue() != null
        ) {
