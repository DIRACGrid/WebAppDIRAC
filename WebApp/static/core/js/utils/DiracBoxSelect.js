...
Ext
		.define(
				'Ext.dirac.utils.DiracBoxSelect',
				{
					...
					/**
					 * @cfg {Boolean} If set to `true`, allows the combo field to hold
					 *      more than one value at a time, and allows selecting multiple
					 *      items from the dropdown list. The combo's text field will show
					 *      all selected values using the template defined by
					 *      {@link #labelTpl}.
					 * 
					 * 
					 */
					 ...
					 *      to use for the inner markup of the labelled items. Defaults to
					 *      the configured {@link #displayField}
					 */
					 ...
					 * @inheritdoc
					 * 
					 * When {@link #forceSelection} is `false`, new records can be created
					 * by the user as they are typed. These records are **not** added to
					 * the combo's store. This creation is triggered by typing the
					 * configured 'delimiter', and can be further configured using the
					 * {@link #createNewOnEnter} and {@link #createNewOnBlur}
					 * configuration options.
					 * 
					 * This functionality is primarily useful with BoxSelect components
					 * for things such as an email address.
					 */
					forceSelection : true,
					 ...
					 * 
					 * With {@link #createNewOnEnter} set to `true`, the creation
					 * described in {@link #forceSelection} will also be triggered by the
					 * 'enter' key.
					 */
					 ...
					 * @cfg {Boolean} Has no effect if {@link #forceSelection} is `true`.
					 * 
					 * With {@link #createNewOnBlur} set to `true`, the creation described
					 * in {@link #forceSelection} will also be triggered when the field
					 * loses focus.
					 * 
					 * Please note that this behavior is also affected by the
					 * configuration options {@link #autoSelect} and {@link #selectOnTab}.
					 * If those are true and an existing item would have been selected as
					 * a result, the partial text the user has entered will be discarded
					 ...
					 * submit as a json encoded array in a single GET/POST variable -
					...
					/**
					 * @cfg {Boolean} `true` to activate the trigger when clicking in
					 *      empty space in the field. Note that the subsequent behavior of
					 *      this is controlled by the field's {@link #triggerAction}.
					 *      This behavior is similar to that of a basic ComboBox with
					 *      {@link #editable} `false`.
					 */
					triggerOnClick : true,

					/**
					 * @cfg {Boolean} - `true` to have each selected value fill to the
					 *      width of the form field - `false to have each selected value
					 *      size to its displayed contents
					 */
					stacked : false,
					...
					/**
					 * @cfg {Boolean} True to hide the currently selected values from the
					 *      drop down list. These items are hidden via css to maintain
					 *      simplicity in store and filter management. - `true` to hide
					 *      currently selected values from the drop down pick list -
					 ...
					 * vertically to its content. Note that this overrides the natural
					 * trigger grow functionality, which is used to size the field
					 * horizontally.
					 */
					grow : true,
					 ...
					 * 
					 * The minimum height to allow when {@link #grow} is `true`, or
					 * `false` to allow for natural vertical growth based on the current
					 * selected values. See also {@link #growMax}.
					 */
					growMin : false,
					 ...
					 * @cfg {Number/Boolean} Has no effect if {@link #grow} is `false`
					 * 
					 * The maximum height to allow when {@link #grow} is `true`, or
					 * `false` to allow for natural vertical growth based on the current
					 * selected values. See also {@link #growMin}.
					 */
					growMax : false,
					 ...
					 *       horizontal growth and BoxSelect only supports vertical
					 *       growth.
					 */
					/**
					 ...
					 *        {@link #grow}/{@link #growMin}/{@link #growMax} configs
					 *        as a result. This event provides a hook for the developer to
					 *        apply additional logic at runtime to resize the field if
					 *        needed.
					 ...
					 * @inheritdoc
					 * 
					 * Initialize additional settings and enable simultaneous typeAhead
					 * and multiSelect support
					 * @protected
					 */
					 ...
					 * @inheritdoc
					 * 
					 * Create a store for the records of our current value based on the
					 * main store's model
					 * @protected
					 */
					 ...
					 * values can span across multiple store page loads and other
					 * filtering. Overlaps some with {@link #isFilteredRecord}, but
					 * findRecord is used by the base component for various logic so this
					 * logic is applied here as well.
					 * 
					 * @protected
					 */
					...
					/**
					 * Overridden to map previously selected records to the "new" versions
					 * of the records based on value field, if they are part of the new
					 * store load
					 * 
					 * @protected
					 */
					...
					/**
					 * Used to determine if a record is filtered out of the current
					 * store's data set, for determining if a currently selected value
					 * should be retained.
					 * 
					 * Slightly complicated logic. A record is considered filtered and
					 * should be retained if: - It is not in the combo store and the store
					 * has no filter or it is in the filtered data set (Happens when our
					 * selected value is just part of a different load, page or query) -
					 * It is not in the combo store and forceSelection is false and it is
					 * in the value store (Happens when our selected value was created
					 * manually)
					 * 
					 * @private
					 */
					 ...
					 * @inheritdoc
					 * 
					 * Overridden to allow for continued querying with multiSelect
					 ...
					 * Otherwise the value of our field will be changing just because our
					 * view of the choices is.
					 * 
					 * @protected
					 */
					...
					/**
					 * Overridden to use valueStore instead of valueModels, for inclusion
					 * of filtered records. See {@link #isFilteredRecord}
					 * 
					 * @private
					 */
					...
					/**
					 * Handles keyDown processing of key-based selection of labelled
					 * items. Supported keyboard controls: - If pick list is expanded -
					 * `CTRL-A` will select all the items in the pick list - If the cursor
					 * is at the beginning of the input field and there are values present -
					 * `CTRL-A` will highlight all the currently selected values -
					 * `BACKSPACE` and `DELETE` will remove any currently highlighted
					 * selected values - `RIGHT` and `LEFT` will move the current
					 * highlight in the appropriate direction - `SHIFT-RIGHT` and
					 * `SHIFT-LEFT` will add to the current highlight in the appropriate
					 * direction
					 * 
					 * @protected
					 */
					...
					/**
					 * Overridden to handle key navigation of pick list when list is
					 * filtered. Because we want to avoid complexity that could be
					 * introduced by modifying the store's contents, (e.g., always having
					 * to search back through and remove values when they might be re-sent
					 * by the server, adding the values back in their previous position
					 * when they are removed from the current selection, etc.), we handle
					 * this filtering via a simple css rule. However, for the moment since
					 * those DOM nodes still exist in the list we have to hijack the
					 * highlighting methods for the picker's BoundListKeyNav to
					 * appropriately skip over these hidden nodes. This is a less than
					 * ideal solution, but it centralizes all of the complexity of this
					 * problem in to this one method.
					 * 
					 * @protected
					 */
					onExpand : function() {
					 ...
					 *             inputEl for rawValue handling, so that we can use
					 *             inputEl for user input of just the current value.
					 */
					 ...
					 * 
					 * @param {Mixed}
					 *          value The value or values to remove from the current
					 *          value, see {@link #setValue}
					 */
					...
					/**
					 * Sets the specified value(s) into the field. The following value
					 * formats are recognised: - Single Values - A string associated to
					 * this field's configured {@link #valueField} - A record containing
					 * at least this field's configured {@link #valueField} and
					 * {@link #displayField} - Multiple Values - If {@link #multiSelect}
					 * is `true`, a string containing multiple strings as specified in the
					 * Single Values section above, concatenated in to one string with
					 * each entry separated by this field's configured {@link #delimiter} -
					 ...
					 * {@link #store}'s configured model type will be created using the
					 * given value as the {@link #displayField} and {@link #valueField}.
					 * This record will be added to the current value, but it will **not**
					 * be added to the store. 2. If {@link #forceSelection} is `true` and
					 * {@link #queryMode} is `remote`, the list of unknown values will be
					 * submitted as a call to the {@link #store}'s load as a parameter
					 * named by the {@link #valueField} with values separated by the
					 * configured {@link #delimiter}. ** This process will cause setValue
					 * to asynchronously process. ** This will only be attempted once. Any
					...
					/**
					 * @inheritdoc Overridden to optionally allow for submitting the field
					...
					/**
					 * Overridden to clear the input field if we are auto-setting a value
					 * as we blur.
					 * 
					 * @protected
					 */
					...
					/**
					 * Expand record values for evaluating change and fire change events
					 * for UI to respond to
					 */
