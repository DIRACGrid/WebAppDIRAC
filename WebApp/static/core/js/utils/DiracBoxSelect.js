...
Ext
	.define(
		'Ext.dirac.utils.DiracBoxSelect',
		{
		    ...
		    /**
		     * @cfg {Boolean} If set to `true`, allows the combo field
		     *      to hold more than one value at a time, and allows
		     *      selecting multiple items from the dropdown list. The
		     *      combo's text field will show all selected values
		     ...
		     *      to use for the inner markup of the labelled items.
		     *      Defaults to the configured {@link #displayField}
		     */
		     ...
		     * @inheritdoc
		     * 
		     * When {@link #forceSelection} is `false`, new records can
		     * be created by the user as they are typed. These records
		     * are **not** added to the combo's store. This creation is
		     * triggered by typing the configured 'delimiter', and can
		     * be further configured using the {@link #createNewOnEnter}
		     * and {@link #createNewOnBlur} configuration options.
		     * 
		     * This functionality is primarily useful with BoxSelect
		     * components for things such as an email address.
		     */
		    forceSelection : true,

		    /**
		     * @cfg {Boolean} Has no effect if {@link #forceSelection}
		     *      is `true`.
		     * 
		     * With {@link #createNewOnEnter} set to `true`, the
		     * creation described in {@link #forceSelection} will also
		     * be triggered by the 'enter' key.
		     */
		     ...
		     *      is `true`.
		     * 
		     * With {@link #createNewOnBlur} set to `true`, the creation
		     * described in {@link #forceSelection} will also be
		     * triggered when the field loses focus.
		     * 
		     * Please note that this behavior is also affected by the
		     * configuration options {@link #autoSelect} and
		     * {@link #selectOnTab}. If those are true and an existing
		     * item would have been selected as a result, the partial
		     * text the user has entered will be discarded and the
		     * existing item will be added to the selection.
		     */
		    createNewOnBlur : false,

		    /**
		     * @cfg {Boolean} Has no effect if {@link #multiSelect} is
		     *      `false`.
		     * 
		    ...
		    /**
		     * @cfg {Boolean} `true` to activate the trigger when
		     *      clicking in empty space in the field. Note that the
		     *      subsequent behavior of this is controlled by the
		     *      field's {@link #triggerAction}. This behavior is
		     *      similar to that of a basic ComboBox with
		     *      {@link #editable} `false`.
		     */
		    triggerOnClick : true,

		    /**
		     * @cfg {Boolean} - `true` to have each selected value fill
		     *      to the width of the form field - `false to have each
		     *      selected value size to its displayed contents
		     */
		    stacked : false,
		     ...
		     *      `false`
		     * 
		     * `true` to keep the pick list expanded after each
		     * selection from the pick list `false` to automatically
		     * collapse the pick list after a selection is made
		     */
		    pinList : true,
		     ...
		     * @cfg {Boolean}
		     * 
		     * `true` if this field should automatically grow and shrink
		     * vertically to its content. Note that this overrides the
		     * natural trigger grow functionality, which is used to size
		     * the field horizontally.
		     */
		    grow : true,

		    /**
		     * @cfg {Number/Boolean} Has no effect if {@link #grow} is
		     *      `false`
		     * 
		     * The minimum height to allow when {@link #grow} is `true`,
		     * or `false` to allow for natural vertical growth based on
		     * the current selected values. See also {@link #growMax}.
		     */
		    growMin : false,
		     ...
		     *      `false`
		     * 
		     * The maximum height to allow when {@link #grow} is `true`,
		     * or `false` to allow for natural vertical growth based on
		     * the current selected values. See also {@link #growMin}.
		     */
		    growMax : false,

		    /**
		     * @cfg growAppend
		     * @hide Currently unsupported by BoxSelect since this is
		     *       used for horizontal growth and BoxSelect only
		    ...
		    //
		    /**
		     * @event autosize Fires when the **{@link #autoSize}**
		     *        function is triggered and the field is resized
		     *        according to the {@link #grow}/{@link #growMin}/{@link #growMax}
		     *        configs as a result. This event provides a hook
		     *        for the developer to apply additional logic at
		     ...
		     * @private
		     */
		    fieldSubTpl : [ '<div id="{cmpId}-listWrapper" class="x-boxselect {fieldCls} {typeCls}">', '<ul id="{cmpId}-itemList" class="x-boxselect-list">',
			    '<li id="{cmpId}-inputElCt" class="x-boxselect-input">', '<input id="{cmpId}-inputEl" type="{type}" ', '<tpl if="name">name="{name}" </tpl>',
			    '<tpl if="value"> value="{[Ext.util.Format.htmlEncode(values.value)]}"</tpl>', '<tpl if="size">size="{size}" </tpl>', '<tpl if="tabIdx">tabIndex="{tabIdx}" </tpl>',
			    '<tpl if="disabled"> disabled="disabled"</tpl>', 'class="x-boxselect-input-field {inputElCls}" autocomplete="off">', '</li>', '</ul>', '</div>', {
				compiled : true,
				disableFormats : true
			    } ],
		     ...
		     * @inheritdoc
		     * 
		     * Initialize additional settings and enable simultaneous
		     * typeAhead and multiSelect support
		     * @protected
		     */
		     ...
		     * @inheritdoc
		     * 
		     * Create a store for the records of our current value based
		     * on the main store's model
		     * @protected
		     */
		     ...
		     * @inheritdoc
		     * 
		     * Add refresh tracking to the picker for selection
		     * management
		     * @protected
		     */
		     ...
		     * @inheritdoc
		     * 
		     * Overridden to avoid use of placeholder, as our main input
		    ...
		    /**
		     * Overridden to search entire unfiltered store since
		     * already selected values can span across multiple store
		     * page loads and other filtering. Overlaps some with
		     * {@link #isFilteredRecord}, but findRecord is used by the
		     * base component for various logic so this logic is applied
		     * here as well.
		     * 
		     * @protected
		     */
		    ...
		    /**
		     * Overridden to map previously selected records to the
		     * "new" versions of the records based on value field, if
		     * they are part of the new store load
		     * 
		     * @protected
		     */
		    onLoad : function() {
		    ...
		    /**
		     * Used to determine if a record is filtered out of the
		     * current store's data set, for determining if a currently
		     ...
		     * @inheritdoc
		     * 
		     * Overridden to allow for continued querying with
		     * multiSelect selections already made
		     * @protected
		     */
		    doRawQuery : function() {
		    ...
		    /**
		     * When the picker is refreshing, we should ignore selection
		     * changes. Otherwise the value of our field will be
		     * changing just because our view of the choices is.
		     * 
		     * @protected
		     */
		    ...
		    /**
		     * Overridden to preserve current labelled items when list
		     * is filtered/paged/loaded and does not include our current
		     * value. See {@link #isFilteredRecord}
		     * 
		     * @private
		     */
		    ...
		    /**
		     * Overridden to use valueStore instead of valueModels, for
		     * inclusion of filtered records. See
		    ...
		    /**
		     * Overridden to preserve scroll position of pick list when
		     * list is realigned
		     */
		    ...
		    /**
		     * Get the current cursor position in the input field, for
		    ...
		    /**
		     * Check to see if the input field has selected text, for
		     * key-based navigation
		     * 
		     * @private
		     */
		    ...
		    /**
		     * Handles auto-selection and creation of labelled items
		     * based on this field's delimiter, as well as the keyUp
		     * processing of key-based selection of labelled items.
		     * 
		     * @protected
		     */
		    ...
		    /**
		     * Handles auto-selection of labelled items based on this
		     * field's delimiter when pasting a list of values in to the
		     * field (e.g., for email addresses)
		     * 
		     * @protected
		     */
		    ...
		    /**
		     * Overridden to handle key navigation of pick list when
		     * list is filtered. Because we want to avoid complexity
		     * that could be introduced by modifying the store's
		     * contents, (e.g., always having to search back through and
		     * remove values when they might be re-sent by the server,
		     * adding the values back in their previous position when
		     * they are removed from the current selection, etc.), we
		     * handle this filtering via a simple css rule. However, for
		     * the moment since those DOM nodes still exist in the list
		     * we have to hijack the highlighting methods for the
		     * picker's BoundListKeyNav to appropriately skip over these
		     * hidden nodes. This is a less than ideal solution, but it
		     * centralizes all of the complexity of this problem in to
		     * this one method.
		     * 
		     * @protected
		     */
		    ...
		    /**
		     * Overridden to get and set the DOM value directly for
		     * type-ahead suggestion (bypassing get/setRawValue)
		     * 
		     * @protected
		     */
		    ...
		    /**
		     * Delegation control for selecting and removing labelled
		     * items or triggering list collapse/expansion
		     * 
		     * @protected
		     */
		    ...
		    /**
		     * Build the markup for the labelled items. Template must be
		     * built on demand due to ComboBox initComponent lifecycle
		     * for the creation of on-demand stores (to account for
		     * automatic valueField/displayField setting)
		     * 
		     * @private
		     */
		    ...
		    /**
		     * Returns the record from valueStore for the labelled item
		     * node
		     */
		    ...
		    /**
		     * @inheritdoc Intercept calls to getRawValue to pretend
		     *             there is no inputEl for rawValue handling, so
		     *             that we can use inputEl for user input of
		    ...
		    /**
		     * @inheritdoc Intercept calls to setRawValue to pretend
		     *             there is no inputEl for rawValue handling, so
		     *             that we can use inputEl for user input of
		     *             just the current value.
		     */
		     ...
		     * 
		     * @param {Mixed}
		     *                value The value or values to add to the
		     *                current value, see {@link #setValue}
		     */
		    ...
		    /**
		     * Removes a value or values from the current value of the
		     * field
		     * 
		     * @param {Mixed}
		     *                value The value or values to remove from
		     *                the current value, see {@link #setValue}
		     */
		     ...
		     * 
		     * In any of the string formats above, the following occurs
		     * if an associated record cannot be found:
		     * 
		     * 1. If {@link #forceSelection} is `false`, a new record of
		     * the {@link #store}'s configured model type will be
		     * created using the given value as the
		     * {@link #displayField} and {@link #valueField}. This
		     * record will be added to the current value, but it will
		     * **not** be added to the store. 2. If
		     * {@link #forceSelection} is `true` and {@link #queryMode}
		     * is `remote`, the list of unknown values will be submitted
		     * as a call to the {@link #store}'s load as a parameter
		     * named by the {@link #valueField} with values separated by
		     * the configured {@link #delimiter}. ** This process will
		     * cause setValue to asynchronously process. ** This will
		     * only be attempted once. Any unknown values that the
		     * server does not return records for will be removed. 3.
		     * Otherwise, unknown values will be removed.
		     * 
		     * @param {Mixed}
		     *                value The value(s) to be set, see method
		    ...
		    /**
		     * @inheritdoc Overridden to optionally allow for submitting
		     *             the field as a json encoded array.
		     */
		    ...
		    /**
		     * Overridden to clear the input field if we are
		     * auto-setting a value as we blur.
		     * 
		     * @protected
		     */
		    mimicBlur : function() {
		    ...
		    /**
		     * Overridden to handle partial-input selections more
		     * directly
		     */
		    ...
		    /**
		     * Expand record values for evaluating change and fire
		     * change events for UI to respond to
		     */
		    ...
		    /**
		     * Overridden to use value (selection) instead of raw value
		     * and to avoid the use of placeholder
		     */
		    ...
		    /**
		     * Overridden to use inputEl instead of raw value and to
		    ...
		    /**
		     * Intercept calls to onFocus to add focusCls, because the
		     * base field classes assume this should be applied to
		     * inputEl
		     */
		    onFocus : function() {
		    ...
		    /**
		     * Intercept calls to onBlur to remove focusCls, because the
		     * base field classes assume this should be applied to
		     * inputEl
		     */
		    onBlur : function() {
		    ...
		    /**
		     * Intercept calls to renderActiveError to add invalidCls,
		     * because the base field classes assume this should be
		     * applied to inputEl
		     */
		    ...
		    /**
		     * Initiate auto-sizing for height based on {@link #grow},
		     * if applicable.
		     */
		    autoSize : function() {
			var me = this, height;
		    ...
		    /**
		     * Track height change to fire {@link #event-autosize}
