...
Ext.define("Ext.dirac.utils.DiracGridPanel", {
  ...
  loadState: function(data) {
    ...
    if (grid) {
      for (var i = 0; i < me.columns.length; i++) {
        ...
        if (
          Ext.Array.contains(grid.columns, col.getSortParam()) ||
          col.getSortParam() in grid.columns
        ) {
    ...
    if (grid && grid.groupers) {
      ...
      for (var i = 0; i < grid.groupers.length; i++) {
        me.getStore().group(
          grid.groupers[i].property,
          grid.groupers[i].direction
        );
      }
    }
  ...
  constructor: function(config) {
    ...
    me.checkboxFunctionDefinition +=
      "var oElems=Ext.query('#" + config.scope.id + " input.checkrow');";
    me.checkboxFunctionDefinition +=
      ...
      if (i == "checkBox") {
        ...
        if ("renderFunction" in config.oColumns[i]) {
          ...
          if (config.oColumns[i]["renderFunction"] == "rendererStatus") {
            ...
            config.oColumns[i]["renderFunction"] == "renderStatusForGivenColor"
          ) {
            func = function(
              value,
              metaData,
              ...
              return me.renderStatusForGivenColor(
                value,
                metaData,
                record,
                rowIndex,
            ...
            var message =
              config.oColumns[i]["renderFunction"] +
              " render function does not exists!!!";
          }
          Ext.apply(oColumn, {
            renderer: func
          });
