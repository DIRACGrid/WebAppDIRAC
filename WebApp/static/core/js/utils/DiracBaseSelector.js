...
Ext.define("Ext.dirac.utils.DiracBaseSelector", {
  ...
  constructor: function(oConfig) {
    ...
    if (oConfig.url == null) {
      GLOBAL.APP.CF.log(
        "error",
        "You have to provide an URL used to fill the selectors!"
      );
    }
    ...
    if (oConfig.textFields) {
      for (field in oConfig.textFields) {
        ...
        if (
          oConfig.textFields[field]["type"] == "number" ||
          oConfig.textFields[field]["type"] == "Number"
        ) {
          ...
          oConfig.textFields[field]["type"] == "Checkbox" ||
          oConfig.textFields[field]["type"] == "checkbox"
        ) {
    ...
    if (data.leftMenu.selectors && me.selectorMenu) {
      for (var i = 0; i < me.selectorMenu.items.length - 1; i++) {
        ...
        if (item.relatedCmbField in data.leftMenu.selectors) {
          ...
          // data
          item.setChecked(
            !data.leftMenu.selectors[item.relatedCmbField].hidden
          );
          ...
          me.__oprPostponedValueSetUntilOptionsLoaded(
            ...
            i == me.selectorMenu.items.length - 2 ||
              Object.keys(data.leftMenu.selectors).length == 1
              ? true
              : false
          );

          me.cmbSelectors[item.relatedCmbField].setInverseSelection(
            data.leftMenu.selectors[item.relatedCmbField].not_selected
          );
        }
      }
    } else {
   ...
   */
  __oprPostponedValueSetUntilOptionsLoaded: function(
    oSelectionBox,
    oValues,
    bLastOne
  ) {
    ...
    if (me.bDataSelectionLoaded) {
      ...
      Ext.Function.defer(
        ...
        1500,
        me,
        [oSelectionBox, oValues, bLastOne]
      );
    }
  },
  /*************************************************************************
  ...
  __cancelPreviousDataRequest: function() {
    ...
    if (me.grid && me.grid.store.loading && me.grid.store.lastDataRequest) {
      ...
      for (id in oRequests) {
        if (
          oRequests.hasOwnProperty(id) &&
          oRequests[id].options == me.grid.store.lastDataRequest.request
        ) {
              ...
              if ("properties" in GLOBAL.USER_CREDENTIALS) {
                for (var i = 0; i < me.properties.length; i++) {
                  if (
                    ...
                    me.cmbSelectors[me.properties[i][2]].setValue([
  ...
  __oprRefreshStoresForSelectors: function(oData, bRefreshStores) {
    ...
    for (var j = 0; j < me.datamap.length; j++) {
      ...
      for (var i = 0; i < oData[me.datamap[j][0]].length; i++)
        dataOptions.push({
          text: oData[me.datamap[j][0]][i][0],
          value: oData[me.datamap[j][0]][i][0]
        });
  ...
  oprLoadGridData: function() {
    ...
    if (me.grid && me.__oprValidateBeforeSubmit()) {
      ...
      var oCheckbox = Ext.query(
        "#" + me.scope.id + " input.dirac-table-main-check-box"
      );
  ...
  disableElements: function(notToDisable) {
    ...
    if (me.disableTextFields) {
      for (var field in me.textFields) {
        if (
          me.textFields[field].canDisable &&
          me.textFields[field].getFieldLabel() != notToDisable.getFieldLabel()
        ) {
