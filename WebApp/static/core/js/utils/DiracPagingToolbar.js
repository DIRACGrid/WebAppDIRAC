...
Ext.define("Ext.dirac.utils.DiracPagingToolbar", {
  ...
  constructor: function(config) {
    ...
    if (config.toolButtons) {
      var nbOfButtons = 0;
      if (
        "Visible" in config.toolButtons &&
        config.toolButtons.Visible.length > 0
      ) {
        for (var i = 0; i < config.toolButtons.Visible.length; i++) {
          if (config.toolButtons.Visible[i].menu) {
            me.pagingToolbarButtons[nbOfButtons] =
              config.toolButtons.Visible[i];
          } else {
            me.pagingToolbarButtons[nbOfButtons] = Ext.create("Ext.Button", {
              text: config.toolButtons.Visible[i].text,
              handler: Ext.bind(
                config.toolButtons.Visible[i].handler,
                config.scope,
                config.toolButtons.Visible[i].arguments,
                false
              )
            });
            if ("properties" in config.toolButtons.Visible[i]) {
              Ext.apply(
                me.pagingToolbarButtons[nbOfButtons],
                config.toolButtons.Visible[i].properties
              );
            }
          }
          nbOfButtons++;
      ...
      if (
        "Protected" in config.toolButtons &&
        config.toolButtons.Protected.length > 0
      ) {
        for (var i = 0; i < config.toolButtons.Protected.length; i++) {
          if (
            ...
            Ext.Array.indexOf(
              ...
              config.toolButtons.Protected[i].property
            ) != -1
          ) {
            if (config.toolButtons.Protected[i].menu) {
              me.pagingToolbarButtons[nbOfButtons] =
                config.toolButtons.Protected[i];
            } else {
              me.pagingToolbarButtons[nbOfButtons] = Ext.create("Ext.Button", {
                text: config.toolButtons.Protected[i].text,
                handler: Ext.bind(
                  config.toolButtons.Protected[i].handler,
                  config.scope,
                  config.toolButtons.Protected[i].arguments,
                  false
                )
              });
              if ("properties" in config.toolButtons.Protected[i]) {
                Ext.apply(
                  me.pagingToolbarButtons[nbOfButtons],
                  config.toolButtons.Protected[i].properties
                );
              }
            }
          }
  ...
  loadState: function(data) {
    ...
    if (data && data.grid && data.grid.pagingToolbar) {
      ...
      if (toolbar.refreshCycle) {
        var refreshCycle =
          toolbar.refreshCycle == 60000 ? 900000 : toolbar.refreshCycle;
