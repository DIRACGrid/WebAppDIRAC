        ...
        var store = Ext.create('Ext.data.TreeStore', {
              ...
              listeners : {
                nodebeforeexpand : function(node, op) {
                  ...
                  if (!forceLoad && activeDesktop && activeDesktop.title == node.get('text') && node.get('text') != 'Default')
                    return;

                  if (node.loaded && node.get('text') == 'Default')
                    return;

                  if (node.get('type') == "desktop") {
                    if (node.get('application') != 'Default') { // trick: When the
                  ...
                  } else if (node.get('type') == 'Default') { // it is the default
                    // desktop
        ...
        me.tree = Ext.create('Ext.tree.Panel', {
              ...
              listeners : {
                itemclick : function(record, item, index, e, eOpts) {
                  ...
                  if ((item.get('expandable')) || (item.get('type') == "Default") || (item.get('text') == "All" && item.get('application') == "Default" && item.parentNode.childNodes.length < 2)) {
                    return;
                  }
