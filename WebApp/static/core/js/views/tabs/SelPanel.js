...
Ext.define("Ext.dirac.views.tabs.SelPanel", {
    ...
    var store = Ext.create("Ext.data.TreeStore", {
      ...
      listeners: {
        nodebeforeexpand: function(node, op) {
          ...
          var forceLoad =
            activeDesktop &&
            node.childNodes.length - 1 != activeDesktop.items.length;
          ...
          if (
            !forceLoad &&
            activeDesktop &&
            activeDesktop.title == node.get("text") &&
            node.get("text") != "Default"
          )
            return;
      ...
      listeners: {
        itemclick: function(record, item, index, e, eOpts) {
          if (
            ...
            (item.get("text") == "All" &&
              item.get("application") == "Default" &&
              item.parentNode.childNodes.length < 2)
          ) {
            return;
          }
          ...
          if (item.data.type == "link") {
            ...
            var activeDesktop = GLOBAL.APP.MAIN_VIEW.getRightContainer().getTabFromApplicationContainer(
              "Default"
            );
            if (activeDesktop == null) {
              ...
              activeDesktop = GLOBAL.APP.MAIN_VIEW.getRightContainer().getTabFromApplicationContainer(
                "Default"
              );
            }
            GLOBAL.APP.MAIN_VIEW.createWindow(
              item.get("type"),
            ...
            item.get("type") == "tabView" ||
            item.get("type") == "presenterView"
          ) {
            if (item.get("isShared") == true) {
              if (item.get("stateType") == "desktop") {
                ...
                var activeDesktop = GLOBAL.APP.MAIN_VIEW.getRightContainer().getTabFromApplicationContainer(
                  item.get("stateToLoad")
                );
                if (activeDesktop == null) {
                  GLOBAL.APP.MAIN_VIEW._state_related_url.push(
                    item.get("stateToLoad")
                  );
                  GLOBAL.APP.MAIN_VIEW.createDesktopTab(
                    item.get("stateToLoad"),
                  ...
                  GLOBAL.APP.MAIN_VIEW.loadSharedStateByName(
                    item.get("application"),
                    item.get("stateToLoad")
                  );
                } else {
                  GLOBAL.APP.MAIN_VIEW.getRightContainer().setActiveTab(
                    activeDesktop
                  );
                }
              } else {
                ...
                // desktop...
                var stateUrl =
                  item.get("application") + ":" + item.get("stateToLoad");
                if (
                  !Ext.Array.contains(
                ...
                var applicationTab = activeDesktop.getApplicationTab(
                  item.get("application"),
                  item.get("stateToLoad")
                );
                if (applicationTab) {
              ...
              if (activeDesktop == null || item.get("text") == "All") {
                GLOBAL.APP.MAIN_VIEW.oprLoadDesktopState(
                  item.get("application"),
                  activeDesktop
                );
              }
        ...
        beforeitemmove: function(node, oldParent, newParent, index, eOpts) {
          if (oldParent.get("text") != newParent.get("text")) {
            ...
            if (!GLOBAL.APP.MAIN_VIEW.isTabOpen(oldDesktopName, tabName)) {
              GLOBAL.APP.MAIN_VIEW.moveApplication(
                tabName,
                moduleName,
                oldDesktopName,
                newDesktopName
  ...
  oprRefreshAppStates: function(appClassName, node) {
    ...
    for (var i = 0, len = oStates.length; i < len; i++) {
      ...
      var data = {
        data: GLOBAL.APP.SM.getStateData(
          "application",
          appClassName,
          stateName
        ),
