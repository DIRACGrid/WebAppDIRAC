...
Ext.define("Ext.dirac.views.tabs.Main", {
  ...
  __oprLoadUrlState: function() {
    ...
    if (GLOBAL.URL_STATE.length != "") {
      ...
      if (parseInt(oParts[0], 10) == 0) {
        GLOBAL.APP.CF.alert(
          "Tab theme can not load applications from URL",
          "error"
        );
        me.loadleftContainer.hide();
        me.loadRightContainer.hide();
  ...
  readLayoutFromStates: function(oDesktop, cbFunction) {
    ...
    for (var i = 0; i < oDesktop.length; i++) {
      ...
      if (oDesktop[i] != "") {
        ...
        switch (iStateLoaded) {
          case 1:
            oStateData = GLOBAL.APP.SM.getStateData(
              "application",
              "desktop",
              oDesktop[i]
            );
            break;
          case -1:
            var isRefernceLoaded = GLOBAL.APP.SM.isStateLoaded(
              "reference",
              "desktop",
              oDesktop[i]
            );
            switch (isRefernceLoaded) {
              ...
              case -1:
                GLOBAL.APP.CF.alert(
                  "The " + oDesktop[i] + " state does not exist !",
                  "warning"
                );
  ...
  loadDefaultdesktop: function(applications, isLoaded, loadFinished, cbLoad) {
      ...
      for (var i = 0; i < applications.length; i++) {
        if (applications[i] != "") {
          ...
          if (
            ...
            Ext.util.Format.trim(application[1]) != ""
          ) {
            // store
          ...
          var isStateLoaded = GLOBAL.APP.SM.isStateLoaded(
            "application",
            application[1],
            "|"
          ); // OK
    ...
    GLOBAL.APP.MAIN_VIEW.createDesktopTab(
      "Default",
      me.view,
      afterDefaultTabCreated
    );
  },
                  ...
                  if (activeTab.currentState == "") {
                    ...
                    Ext.defer(function() {
                      ...
                      tab.items.each(function(win, value, length) {
                        if (
                          activeTab.currentState == win.currentState &&
                          activeTab.name == win.setupData.name
                        ) {
  ...
  createTreeMenu: function() {
    ...
    for (var j = 0; j < GLOBAL.APP.configData["menu"].length; j++) {
      me.__getAppRecursivelyFromConfig(
        GLOBAL.APP.configData["menu"][j],
        rootNode
      );
    }
  ...
  __getAppRecursivelyFromConfig: function(item, rootNode) {
    ...
    if (item.length == 2) {
      ...
      if (item[0] == "app") {
        ...
        var sStartClass =
          oParts.length == 2 ? item[2] + ".classes." + oParts[1] : item[2];
  ...
  createDesktopTree: function(node) {
    ...
    for (var i = 0, len = oStates.length; i < len; i++) {
      ...
      var oStateData = GLOBAL.APP.SM.getStateData(
        "application",
        "desktop",
        sStateName
      );
  ...
  oprLoadSharedDesktopsAndApplications: function() {
    ...
    for (var i = 0; i < applications.length; i++) {
      ...
      var appRefs = GLOBAL.APP.SM.getApplicationStates(
        "reference",
        applications[i]
      ); // OK
      if (appRefs.length > 0) {
  ...
  createWindow: function(
    ...
    if (loadedObjectType == "app") {
      ...
      var sStartClass =
        oParts.length == 2 ? moduleName + ".classes." + oParts[1] : moduleName;
      ...
      if (appl) {
        GLOBAL.APP.MAIN_VIEW.SM.saveAsState(
          ...
          function(desktop, app, stateName) {
            Ext.dirac.system_info.msg(
              "Notification",
              stateName + " application is saved on " + desktop + "!"
            );
            ...
            if (desktop != "Default" && appl.currentState != stateName) {
              ...
              appl.setTitle(
                appl.loadedObject.launcher.title +
                  " [" +
                  appl.loadedObject.currentState +
                  "]"
              );
  ...
  addNodeToMenu: function(stateName, appName) {
    ...
    if (selPanel) {
      ...
      /* eslint-disable */
      if (
        !Ext.data.schema.Schema.instances.default.hasEntity(
          "Ext.dirac.views.tabs.TreeNodeModel"
        )
      ) {
        /* eslint-enable */
        ...
        /* eslint-disable */
        var model = Ext.data.schema.Schema.instances.default.getEntity(
          "Ext.dirac.views.tabs.TreeNodeModel"
        ); // returns the constructor MyApp.models.MyModel()
        /* eslint-enable */
  ...
  deleteApplicationStates: function() {
    ...
    if (activeDesktop) {
      ...
      if (appl) {
        ...
        GLOBAL.APP.MAIN_VIEW.SM.formManageStates(
          appl.loadedObject.self.getName(),
          funcAfterRemove
        );
      } else {
    ...
    if (selPanel) {
      ...
      if (oNode) {
        ...
        try {
          ...
          if (
            !Ext.data.schema.Schema.instances.default.hasEntity(
              "Ext.dirac.views.tabs.DesktopNodeModel"
            )
          ) {
            /* eslint-enable */
  ...
  addToSharedApplications: function(applicationName, stateName, stateType) {
    ...
    if (selPanel) {
      ...
      var qtip =
        application[application.length - 1] + "<br>State Name: " + stateName;
      try {
        rootNode.appendChild({
          text: stateName,
          expandable: false,
  ...
  __addUserDesktopStatesToDesktop: function(rootNode) {
    ...
    switch (iStateLoaded) {
      case -1:
        GLOBAL.APP.CF.alert(
          "The state " + sStateName + " does not exist !",
          "warning"
        );
        return;
        break;
    ...
    var oDesktop = GLOBAL.APP.SM.getStateData(
      "application",
      "desktop",
      sStateName
    );
    ...
    if (me.currentState != "") {
      ...
      if (me.currentState != "Default") {
        ...
        if (activeDesktop && activeDesktop.isLoaded) {
          ...
          default_State_url +=
            me._default_desktop_state.length < 1 ? "**," : "";
        }
      }
      ...
      if (
        stateName != "" &&
        !Ext.Array.contains(me._state_related_url, stateName)
      ) {
        if (me._state_related_url.length > 0) {
  ...
  __loadDesktopStateData: function(sStateName, tab) {
    ...
    switch (iStateLoaded) {
      case 1:
        var state = GLOBAL.APP.SM.getStateData(
          "application",
          "desktop",
          sStateName
        );
  ...
  createNewDesktop: function() {
    ...
    GLOBAL.APP.MAIN_VIEW.SM.formSaveDialog(
      "application",
      "desktop",
      null,
      afterSave,
  ...
  moveApplication: function(
    ...
    GLOBAL.APP.MAIN_VIEW.SM.moveAppState(
      applicationName,
      module,
      oldDesktopName,
      newDesktopName
  ...
  openHelpWindow: function(app) {
    ...
    if (app.appClassName == "link") {
      Ext.dirac.system_info.msg(
        "Error",
        "You can not add help to an external link!"
      );
    } else {
