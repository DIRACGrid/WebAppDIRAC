/***
 * This class is a container which contains the left and right panels.
 * The desktop split into two parts:
 * 1. The left panel contains a menu used to manage the applications.
 * 2. The right panel contains the opened applications
 */
...
//layout: 'fit',
  ...
  __oprLoadUrlState : function() {
    ...
    if (oValid) {
      ...
      if (parseInt(oParts[0], 10) == 1) {
        // desktop state
        me.loading = true; //it is only used by the tabchange event in order to not load the applications.
  ...
  /***
   * This function is used to read the saved desktop states from the server or the cache using SM.
   * #param{Ext.Array} oDesktop is a list of desktop names which are in the URL.
   * #param{Object} cbFunction is a function which will be called after the desktop states active.
   */
  ...
  loadDesktopStateData : function(sStateName, tab){
    var me = this;
    var iStateLoaded = GLOBAL.APP.SM.isStateLoaded("application", "desktop", sStateName);

    switch (iStateLoaded) {
    case -1:
      GLOBAL.APP.CF.alert("The "+sStateName+" state does not exist !", "warning");
      return;
      break;
    case -2:
      me.funcPostponedLoading = function() {

      me.loadDesktopStateData(sStateName, tab);
   ...
   * The tree has application nodes, My desktop node and Shared desktop node.
   */
  createTreeMenu : function() {
    ...
    var oFuncMyDesktop = function(iCode, sAppName) {
      me.createDesktopTree(rootNode);
    };

    var oFuncSharedDesktop = function(iCode, sAppName) {
      me.createSharedDesktop(rootNode);
    };

    if (GLOBAL.STATE_MANAGEMENT_ENABLED)
      GLOBAL.APP.SM.oprReadApplicationStatesAndReferences("desktop", oFuncMyDesktop);// OK
    GLOBAL.APP.SM.oprReadSharedDesktops("desktop", oFuncSharedDesktop)
   ...
   * @param {Object} an instance of a node
   */
  ...
  /***
   * It creates the Shared desktop sub-tree.
   * @param {Object} an instance of a node
   */
  createSharedDesktop : function(node){
    var me = this;
    var rootNode = node.appendChild({
    ...
    var username = GLOBAL.APP.configData.user.username;
    var oSharedDesktops = GLOBAL.APP.SM.getApplicationStates('sharedDesktop', 'desktop');
    var mydesktop = GLOBAL.APP.SM.getStateData('sharedDesktop', 'desktop', username);
    if (mydesktop){
      for ( var stateName in mydesktop) {
        if (stateName != 'Metadata'){
          //when the user clicks on the desktop name then it will open.
          var childNode = rootNode.appendChild({
            'text' : stateName,
            expandable : false,
    ...
    var shareddesktop = rootNode.appendChild({
      'text' : 'Others',
      expandable : true,
      leaf : false,
      allowDrag : false,
    ...
    for ( var i = 0, len = oSharedDesktops.length; i < len; i++) {
      desktop = oSharedDesktops[i];
      if (desktop != username){
        var desktops = GLOBAL.APP.SM.getStateData('sharedDesktop', 'desktop', desktop);
        for ( var stateName in desktops) {
          if (stateName != 'Metadata'){
            var childNode = shareddesktop.appendChild({
              'text' : stateName,
              expandable : true,
              application : stateName,
              type : 'desktop',
  ...
  /***
   *It changes the workspace in the RightContainer taking account what was selected in the LeftConatiner.
   *@param {String} type can be welcome, menuPanel and sharedLayouts.
   */
   ...
   *@param{String} loadedObjectType is the type of the object which will be loaded
   *@param{String} moduleName is the full package of the class for example: DIRAC.JobMonitor.classes.JobMonitor
   ...
   *@param{String} view is used to know the type of the desktop. NOTE: It can be tab or presenter like
  ...
  /**
   * It saves the state of an existing application with a new name.
   * Note: It only take into account the active a application.
   */
  ...
  refreashTree : function(desktop, application) {
    //<debug>
    Ext.log({level:'log',stack:true},"Begin method!");
    //</debug>
    ...
    if (selPanel) {
      ...
      'The panel which contains the menu does not exists!!!');
    }
    //<debug>
    Ext.log({level:'log',stack:true},"End method!");
    //</debug>
      ...
      if (rootNode.childNodes.length > 0) {
        for ( var i = 0; i < rootNode.childNodes.length; i++) {
          ...
          if (node.data.text == desktop && desktop!='Default') {
            ...
            parent = node.parentNode; // the application found. the
  ...
  /**
   * It saves the desktop state. If the state name is defaulf, a new
   * must be defined.
   */
   ...
   *
   * @param {string} name the new name of the desktop.
   * @param {boolean} addToMenu if it is set, it means the new name
   *           has to be added to the menu.
   */
  ...
  __addDesktopToMenu : function(stateName) {
    ...
    if (selPanel) {
      ...
      var oNode = null;
      rootNode.eachChild(function(node) { //remove the node if it is exists
   ...
   *
   * @param {String} stateName it is the desktop name which has to be
  ...
  /**
   * It adds a node to the Shared Desktops node. The name of the node is the stateName.
   * @param{String} stateName
   */
   ...
   * @param desktopName {String} the name of the desktop
   * @param tabName {String} the name of the tab (window) whih needs to be closed.
   */
  ...
  /**
   * It retrieves the desktop states from the the UserProfile making a request to the controller.
   * It adds the applications states to the specific node.
   * @private
   * @property {Ext.dirac.views.tabs.TreeMenuModel} rootNode the name of the node which is the desktop name.
   */
