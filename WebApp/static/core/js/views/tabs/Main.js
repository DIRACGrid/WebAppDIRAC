...
Ext.define('Ext.dirac.views.tabs.Main', {
  ...
  requires : ['Ext.dirac.views.tabs.LeftContainer', 'Ext.dirac.views.tabs.RightContainer',
      'Ext.dirac.views.tabs.TreeMenuModel', 'Ext.dirac.views.tabs.StateManagement'],
  ...
  /*****************************************************************************
   * This function is used to read the saved desktop states from the server or
   * the cache using SM. #param{Ext.Array} oDesktop is a list of desktop names
   * which are in the URL. #param{Object} cbFunction is a function which will be
   * called after the desktop states active.
   */
  ...
  /**
   * It creates a tree which nodes are applications. This is used as a Menu. The
   * tree has application nodes, My desktop node and Shared desktop node.
   */
  createTreeMenu : function() {
    ...
    var oFuncMyDesktop = function(iCode, sAppName) {
      ...
      me.loadRightContainer.iCode = iCode;
      me.loadlefttContainer.iCode = iCode;
    };
    ...
    if (mydesktop) {
      for (var stateName in mydesktop) {
        if (stateName != 'Metadata') {
          // when the user clicks on the desktop name then it will
          // open.
          var childNode = rootNode.appendChild({
                'text' : stateName,
                expandable : false,
   ...
   * @param{String} tabName is the tab name which will contain the application
   *                which being loaded.
   */
   ...
   * @param{String} view is used to know the type of the desktop. NOTE: It can
   *                be tab or presenter like
  ...
  /**
   * It saves the state of an application. Note: It only take into account the
   * active a application.
   */
  saveActiveApplicationState : function() {
    ...
    if (activeDesktop) {
      ...
      if (appl) {
        ...
        GLOBAL.APP.MAIN_VIEW.SM.oprSaveAppState("application", appl.loadedObject.self.getName(), appl.loadedObject,
            funcAfterSave);
        ...
        Ext.dirac.system_info
            .msg("Error",
                'You do not have any active tab on the desktop. Please select a tab (application) in the current desktop!!!');
      }
    } else {
  ...
  saveAsActiveApplicationState : function() {
    ...
    if (activeDesktop) {
      ...
      if (appl) {
        ...
        GLOBAL.APP.MAIN_VIEW.SM.formSaveState("application", appl.loadedObject.self.getName(), appl.loadedObject,
            funcAfterSave);
   ...
   * @param {boolean}
   *          addToMenu if it is set, it means the new name has to be added to
   *          the menu.
   */
   ...
   *
   * @param {String}
   *          stateName it is the desktop name which has to be deleted from the
   *          menu.
   */
  ...
  /**
   * It retrieves the desktop states from the the UserProfile making a request
   * to the controller. It adds the applications states to the specific node.
   *
   * @private
  ...
  /*****************************************************************************
   * @param{Object}oData It contains the information what is needed to open an
   *                     application. -objectType -moduleName -setupData
   */
