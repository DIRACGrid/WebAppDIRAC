...
Ext.define('Ext.dirac.views.tabs.TabPanel', {
  ...
  listeners : {
    ...
    'close' : function() {
      ...
      GLOBAL.APP.SM.oprRemoveActiveState("desktop", me.title); // We have to
      // remove the
      // desktop state
      // from the
      // list.
    ...
    'tabchange' : function(tabPanel, newCard, oldCard, eOpts) {
      var me = this;
      /*
       * if (oldCard != null) { GLOBAL.APP.SM.oprRemoveActiveState("desktop",
       * oldCard.title);// OK }
       */
      if (newCard.type == 'desktop') { // Ignore the change of the applications

        GLOBAL.APP.MAIN_VIEW.currentState = newCard.title; // as we work more
        // than one desktop,
        // we need to set
        // the current state
        // each time when a
