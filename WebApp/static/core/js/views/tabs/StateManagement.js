...
Ext.define("Ext.dirac.views.tabs.StateManagement", {
        ...
        keypress: function(oTextField, e, eOpts) {
          if (e.getCharCode() == 13) {
            if (me.txtStateName.isValid()) {
              ...
              GLOBAL.APP.SM.oprSendDataForSave(
                sAppName,
                oAppObject,
                sStateType,
                sStateName,
   ...
   *          saved
   */
  formSaveAsStateOfData: function(
    desktop,
    selectedStateName,
    sStateType,
    sAppName,
        ...
        keypress: function(oTextField, e, eOpts) {
          if (e.getCharCode() == 13) {
            if (me.txtStateName.isValid()) {
              ...
              var desktops = GLOBAL.APP.SM.getStateData(
                "application",
                "desktop",
                desktop
              );
              ...
              for (var i = 0; i < desktops.data.length; i++) {
                if (
                  desktops.data[i].currentState == selectedStateName &&
                  desktops.data[i].module == sAppName
                ) {
              ...
              me.oprSendDataForSave(
                "desktop",
                desktops,
                "application",
                desktop,
                function(iCode, appName, stateType, stateName) {
  ...
  oprSaveAppState: function(sStateType, sAppName, oAppObject, cbAfterSave) {
    ...
    if (
      oAppObject.title == "" ||
      oAppObject.title == "Default" ||
      oAppObject.currentState == ""
    ) {
  ...
  formManageStates: function(sAppName, cbAfterRemove) {
    ...
    var oPanel = new Ext.create("Ext.panel.Panel", {
      ...
      items: [
          ...
          items: [
            {
              ...
              listeners: {
                change: function(cmp, newValue, oldValue, eOpts) {
                  var oSelectElStates = me.manageWindow.items
                    .getAt(1)
                    .items.getAt(1);
                  var oSelectElLinks = me.manageWindow.items
                    .getAt(1)
                    .items.getAt(2);
        ...
        {
          html:
            "<select multiple='multiple' style='width:100%;height:175px'></select>",
          xtype: "box"
        },
        {
          html:
  ...
  oprFillSelectFieldWithStates: function() {
    var me = this;
    var oSelectEl = document
      .getElementById(me.manageWindow.getId())
    ...
    var oAppStates = GLOBAL.APP.SM.getApplicationStates(
      "application",
      sAppName
    );
  ...
  oprDeleteSelectedStates: function() {
    ...
    var oSelectField = document
      .getElementById(me.manageWindow.getId())
      .getElementsByTagName("select")[iWhoSelect];
          ...
          if (!GLOBAL.APP.SM.isAnyActiveState(sAppName, oStateName)) {
            var cbFunc = function(rCode, rAppName, rStateType, rStateName) {
              if (rCode == 1) {
                me.manageWindow.items
                  .getAt(1)
                  .cbAfterRemove("application", rAppName, rStateName);
            ...
            GLOBAL.APP.CF.alert(
              "The state <b>" +
                oSelectField.options[i].value +
                "</b> you are willing to delete is curently in use !",
              "warning"
            );
        } else {
          var cbFunc = function(rCode, rAppName, rStateType, rStateName) {
            if (rCode == 1) {
              me.manageWindow.items
                .getAt(1)
                .cbAfterRemove("reference", rAppName, rStateName);
              for (var j = oSelectField.length - 1; j >= 0; j--) {
                if (rStateName == oSelectField.options[j].value) {
                  oSelectField.remove(j);
                  break;
  ...
  formStateLoader: function(cbAfterLoad, cbAfterSave) {
    ...
    me.txtLoadText = Ext.create("Ext.form.field.Text", {
      ...
      validate: function() {
        ...
        if (
          Ext.util.Format.trim(sValue) != "" &&
          sValue.split("|").length == 4
        ) {
          return true;
        } else {
          GLOBAL.APP.CF.alert(
            "The value in the 'Shared State' field is not valid !",
            "warning"
          );
          return false;
        }
    ...
    me.btnLoadSharedState = new Ext.Button({
      ...
      handler: function() {
        if (me.txtLoadText.validate()) {
          ...
          GLOBAL.APP.SM.oprLoadSharedState(
            me.txtLoadText.getValue(),
            cbAfterLoad,
            me.txtRefName.getValue()
          );
    ...
    me.btnSaveSharedState = new Ext.Button({
      ...
      handler: function() {
        ...
        if (oValid) {
          GLOBAL.APP.SM.oprSaveSharedState(
            me.txtRefName.getValue(),
            me.txtLoadText.getValue(),
            cbAfterSave
          );
  ...
  deleteStateFromDesktop: function(desktop, appName, stateName, cbAfterSave) {
    ...
    if (!GLOBAL.APP.SM.isAnyActiveState(appName, stateName, cbAfterSave)) {
      ...
      for (var i = 0; i < desktops.data.length; i++) {
        if (
          desktops.data[i].currentState == stateName &&
          desktops.data[i].module == appName
        ) {
      ...
      if (tab && tab.isLoaded) {
        ...
        if (appTab && appTab.isLoaded) {
          if (desktop == "Default") {
            ...
            GLOBAL.APP.MAIN_VIEW.SM.oprSaveAppState(
              "application",
            ...
            if (desktops != -1) {
              for (var i = 0; i < desktops.data.length; i++) {
                if (
                  desktops.data[i].module == app &&
                  desktops.data[i].currentState == state
                ) {
                  ...
                  if (appTab.childWindows.length > 0) {
                    ...
                    for (var i = 0; i < appTab.childWindows.length; i++) {
                      if (appTab.childWindows[i].type == "help") {
                        ...
                        // from the notepad...
                        var helptext = {
                          helptext: appTab.childWindows[i].items
                            .getAt(0)
                            .getValue()
                        };
                    ...
                    Ext.apply(
                      desktops.data[i].data,
                      appTab.loadedObject.getHelpText()
                    );
                  }
                  found = true;
                  break;
            ...
            if (!found) {
              me.formSaveStateOfData(
                desktop,
                appTab,
                "application",
                app,
                function(returnCode, sAppName, sStateType, sStateName) {
                  // cbFunc(desktop, app, sStateName);
                  if (
                    returnCode == 1 &&
                    appTab.loadedObject.currentState != sStateName
                  ) {
                    GLOBAL.APP.SM.oprRemoveActiveState(
                      sAppName,
                      appTab.loadedObject.currentState
                    );
                    ...
                    appTab.setTitle(
                      appTab.loadedObject.launcher.title +
                        " [" +
                        appTab.loadedObject.currentState +
                        "]"
                    );
                  ...
                  GLOBAL.APP.MAIN_VIEW.addApplicationToDesktopMenu(
                    desktop,
                    sStateName,
                    app
                  );
          ...
          Ext.dirac.system_info.msg(
            "Notification",
            state + " is not modified! It is already saved!"
          );
        }
      } else {
      ...
      if (tab && tab.isLoaded) {
        ...
        if (appTab && appTab.isLoaded) {
          if (desktop == "Default") {
            ...
            GLOBAL.APP.MAIN_VIEW.SM.formSaveState(
              "application",
              app,
              appl.loadedObject,
              funcAfterSave
            ...
            me.formSaveAsStateOfData(
              desktop,
              state,
              "application",
              app,
              function(iCode, sAppName, sStateType, sStateName) {
                cbFunc(desktop, app, sStateName);
                GLOBAL.APP.MAIN_VIEW.addApplicationToDesktopMenu(
                  desktop,
                  sStateName,
          ...
          me.formSaveAsStateOfData(desktop, state, "application", app, function(
            iCode,
            sAppName,
            sStateType,
            sStateName
  ...
  __changeActiveState: function(appObj, appName, stateType, stateName) {
    ...
    GLOBAL.APP.MAIN_VIEW.getRightContainer().addStateToExistingWindows(
      stateName,
      appName
    );
    ...
    appObj.setTitle(
      appObj.loadedObject.launcher.title +
        " [" +
        appObj.loadedObject.currentState +
        "]"
    );
    ...
    if (GLOBAL.APP.MAIN_VIEW.SM.saveWindow)
      GLOBAL.APP.MAIN_VIEW.SM.saveWindow.close();
    Ext.Array.remove(
      GLOBAL.APP.MAIN_VIEW._default_desktop_state,
      oldApplicationUrl
    );
            ...
            if (me.txtStateName.isValid()) {
              ...
              me.oprSendDataForSave(
                ...
                function(iCode, appName, stateType, stateName) {
                  cbAfterSave(
                    iCode,
                    appObj.loadedObject.self.getName(),
                    stateType,
                    sStateName
                  );
                }
  ...
  moveAppState: function(
    ...
    if (fromDesktopName == "Default") {
      ...
      var fromDsktop = GLOBAL.APP.SM.getStateData(
        "application",
        "desktop",
        fromDesktopName
      );
      if (fromDsktop != -1) {
        for (var i = 0; i < fromDsktop.data.length; i++) {
          if (
            fromDsktop.data[i].module == moduleName &&
            fromDsktop.data[i].currentState == stateName
          ) {
