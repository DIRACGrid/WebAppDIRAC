...
Ext.define("Ext.dirac.views.tabs.StateManagement", {
  requires: ["Ext.form.field.Text", "Ext.form.field.Date", "Ext.button.Button", "Ext.toolbar.Toolbar", "Ext.panel.Panel", "Ext.window.Window"],

  /**
   * Function called when the create downtime is clicked
   *
   * @param {String}
   *          sAppName Application class name
   */
  formSetDowntime: function(sAppName) {
    var me = this;

    me.txtReason = Ext.create("Ext.form.field.Text", {
      fieldLabel: "Reason:",
      labelAlign: "left",
      margin: 10,
      width: 400,
      ...
      validateValue: function(sValue) {
        ...
        if (sValue.length < 1) {
          this.markInvalid("You must specify a reason!");
          return false;
        }
      },
      ...
      listeners: {
        keypress: function(oTextField, e, eOpts) {
          if (e.getCharCode() == 13) {
            if (me.txtReason.isValid()) {
              var message = me.txtReason.getValue();
            }
          }
        }
      }
    ...
    me.btnAddDowntime = new Ext.Button({
      text: "Create downtime",
      margin: 3,
      iconCls: "dirac-icon-save",
      handler: function() {
        if (me.txtReason.isValid()) {
          Ext.Ajax.request({
            url: GLOBAL.BASE_URL + "Downtimes/setApplicationDowntime",
            params: {
              app: sAppName,
              message: me.txtReason.getValue(),
              start: me.calendarFrom.getValue(),
              end: me.calendarTo.getValue()
            },
            scope: me,
            success: function(response) {
              GLOBAL.APP.CF.msg('Downtime added!')
            },
    ...
    me.btnCancel = new Ext.Button({
      ...
      handler: function() {
        me.txtReason.setValue("");
        me.start.setValue("");
        me.end.setValue("");
        me.setDowntimeWindow.hide();
      },
      scope: me
    });
    ...
    oToolbar.add([me.btnAddDowntime, me.btnCancel]);

    var oPanel = new Ext.create("Ext.panel.Panel", {
      autoHeight: true,
      border: false,
      items: [oToolbar, me.txtReason]
    });

    // initializing window showing the saving form
    me.setDowntimeWindow = Ext.create("widget.window", {
      height: 120,
      width: 500,
      title: "Set downtime",
      layout: "fit",
