...
Ext.define("Ext.dirac.views.tabs.SettingsPanel", {
    ...
    Ext.Ajax.request({
      ...
      success: function(response) {
        if (!response.status == 200) {
          ...
          if (!result.OK) {
            return GLOBAL.APP.CF.alert(
              "Authentication was ended with error: \n" + result.Message,
              "error"
            );
            ...
            if (!result.Value.URL || !result.Value.Session) {
              return GLOBAL.APP.CF.alert(
                "We cannot get authorization URL.",
                "error"
              );
            } else {
              ...
              GLOBAL.APP.CF.log(
                "debug",
                'Open authorization URL: "' + authorizationURL + '"'
              );
              var oAuthReqWin = open(
                authorizationURL,
                "popupWindow",
                "hidden=yes,height=570,width=520,scrollbars=yes,status=yes"
              );
              oAuthReqWin.focus();

              // Send request to redirect URL about success authorization
              Ext.get("app-dirac-loading").show();
              Ext.get("app-dirac-loading-msg").setHtml(
                "Waiting when authentication will be finished..."
              );
              GLOBAL.APP.CF.log(
                "debug",
                "Watch when popup window will be close"
              );
                ...
                if (r === "closed") {
                  return Ext.Ajax.request({
                    ...
                    success: function(response) {
                      ...
                      if (!result.OK) {
                        ...
                        if (result.Value.Status == "failed") {
                          ...
                          result.Value.Status == "authed and reported"
                        ) {
                    ...
                    failure: function(form, action) {
                      ...
                      return GLOBAL.APP.CF.alert(
                        "Request was ended with error: " + form + action,
                        "error"
                      );
                    }
                  });
            ...
            return GLOBAL.APP.CF.alert(
              "Cannot submit authorization flow.",
              "error"
            );
          }
        }
  ...
  addGroupsButton: function() {
    ...
    for (
      var i = 0;
      i < Object.keys(GLOBAL.APP.configData["groupsStatuses"]).length;
      i++
    ) {
      if (
        Object.values(GLOBAL.APP.configData["groupsStatuses"])[i]["Status"] ==
        "ready"
      ) {
        button_group.menu.push({
          ...
          handler: function() {
            ...
            if (oQPosition != -1) {
              location.href =
                oHref.substr(0, oQPosition) + "changeGroup?to=" + me.text;
            } else {
        ...
        button_group.menu.push({
          title: Object.values(GLOBAL.APP.configData["groupsStatuses"])[i][
            "Status"
          ],
          msg: Object.values(GLOBAL.APP.configData["groupsStatuses"])[i][
            "Comment"
          ],
          text: Object.keys(GLOBAL.APP.configData["groupsStatuses"])[i],
      ...
      button_theme.menu.push({
        ...
        handler: function() {
          ...
          if (GLOBAL.APP.MAIN_VIEW._state_related_url.length > 0) {
            sState_related_url =
              "&url_state=1|" + GLOBAL.APP.MAIN_VIEW._state_related_url[0];
            for (
              var i = 1;
              i < GLOBAL.APP.MAIN_VIEW._state_related_url.length;
              i++
            ) {
              sState_related_url +=
                "," + GLOBAL.APP.MAIN_VIEW._state_related_url[i];
            }
          }
          if (oQPosition != -1) {
            location.href =
              oHref.substr(0, oQPosition) +
              "?theme=" +
              me.text +
              "&view=" +
              GLOBAL.VIEW_ID +
      ...
      button_views.menu.push({
        ...
        handler: function() {
          ...
          if (oQPosition != -1) {
            ...
            location.href =
              oHref + "?view=" + me.text + "&theme=" + sButtonThemeText;
          }
        }
      });
    }
