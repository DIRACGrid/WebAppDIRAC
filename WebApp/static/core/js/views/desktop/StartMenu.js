...
Ext.define("Ext.dirac.views.desktop.StartMenu", {
  ...
  initComponent: function() {
    ...
    me.title = GLOBAL.APP.configData.user.username
      ? GLOBAL.APP.configData["user"]["username"] +
        "@" +
        GLOBAL.APP.configData["user"]["group"]
      : "Anonymous";
    me.callParent();
  },
    ...
    if (GLOBAL.STATE_MANAGEMENT_ENABLED) {
      me.add([
        "-",
        {
          ...
          handler: function() {
            GLOBAL.APP.MAIN_VIEW.SM.formStateLoader(
              GLOBAL.APP.MAIN_VIEW.cbAfterLoadSharedState,
              GLOBAL.APP.MAIN_VIEW.cbAfterSaveSharedState
            );
          }
        }
      ]);
        ...
        return {
          ...
          listeners: {
            render: function(oMenuItem, eOpts) {
              GLOBAL.APP.MAIN_VIEW.registerStartAppMenu(
                oMenuItem,
                oMenuItem.appClassName
              );
            ...
            if (stateType == "application") {
              ...
              oNewItem = Ext.create("Ext.menu.Item", {
                ...
                handler: Ext.bind(
                  ...
                  GLOBAL.APP.MAIN_VIEW,
                  [oThisMenu.appClassName, stateName],
                  false
                ),
                scope: me,
            ...
            switch (stateType) {
              case "application":
                for (var i = 0; i < me.menu.items.length; i++) {
                  if (
                    me.menu.items.getAt(i).self.getName() ==
                    "Ext.menu.Separator"
                  )
                    break;
