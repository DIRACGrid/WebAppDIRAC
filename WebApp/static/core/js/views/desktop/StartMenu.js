...
Ext.define("Ext.dirac.views.desktop.StartMenu", {
  ...
  initComponent: function() {
    ...
    me.title = GLOBAL.APP.configData.user.username
      ? GLOBAL.APP.configData["user"]["username"] + "@" + GLOBAL.APP.configData["user"]["group"]
      : "Anonymous";
    me.callParent();
  },
  ...
  getMenuStructureRec: function(item) {
    ...
    if (item.length == 2) {
      ...
      if (item[0] == "app") {
        ...
        var sStartClass = "";
        if (oParts.length == 2) sStartClass = item[2] + ".classes." + oParts[1];
        else sStartClass = item[2];
            ...
            switch (stateType) {
              case "application":
                for (var i = 0; i < me.menu.items.length; i++) {
                  if (me.menu.items.getAt(i).self.getName() == "Ext.menu.Separator") break;
        ...
        return {
          text: item[1],
          handler: Ext.bind(GLOBAL.APP.MAIN_VIEW.createWindow, GLOBAL.APP.MAIN_VIEW, [
            item[0],
            item[2],
            {
              title: item[1]
