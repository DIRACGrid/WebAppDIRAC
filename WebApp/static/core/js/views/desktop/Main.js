...
Ext.define("Ext.dirac.views.desktop.Main", {
  ...
  __oprLoadUrlState: function() {
    ...
    if (oValid) {
      ...
      if (oParts.length != 2 || Ext.util.Format.trim(oParts[1]).length == 0) return;
      ...
      if (parseInt(oParts[0], 10) == 0) {
        ...
        for (var i = 0, len = oApps.length; i < len; i++) {
          ...
          if (Ext.util.Format.trim(oAppItems[1]) != "") oSetupData.stateToLoad = oAppItems[1];
  ...
  createDesktopMenu: function() {
    ...
    if (GLOBAL.STATE_MANAGEMENT_ENABLED) GLOBAL.APP.SM.oprReadApplicationStatesAndReferences("desktop", oFunc); // OK
  ...
  onWindowClose: function(win) {
     ...
     */
    if (!win.isChildWindow) {
      if (win.currentState != "") GLOBAL.APP.SM.oprRemoveActiveState(win.loadedObject.self.getName(), win.currentState); // OK
  ...
  addStateToExistingWindows: function(stateType, stateName, appName) {
    ...
    if (appName in me.registerStartMenus) me.registerStartMenus[appName].addNewState(stateType, stateName);
  },
  ...
  oprRefreshAllAppStates: function(appName) {
    ...
    var oFunc = function(iCode, sAppName) {
      if (iCode == 1) {
        ...
        if (appName in me.registerStartMenus) me.registerStartMenus[appName].oprRefreshAppStates();
      }
    };
  ...
  removeStateFromWindows: function(sStateType, sAppName, sStateName) {
    ...
    if (sAppName in me.registerStartMenus) me.registerStartMenus[sAppName].removeState(sStateType, sStateName);
  },
  ...
  removeDesktopReference: function() {
    ...
    for (var i = me.statesMenu.items.length - 1; i >= 0; i--) {
      if (me.statesMenu.items.getAt(i).self.getName() == "Ext.menu.Separator") break;
  ...
  refreshUrlDesktopState: function() {
    ...
    if (me.currentState != "") {
      ...
      for (var i = 0; i < me.windows.getCount(); i++) {
        ...
        if (oWin != undefined && oWin != null && !oWin.isChildWindow) sNewUrlState += (sNewUrlState == "" ? "" : "^") + oWin.getUrlDescription();
      }
