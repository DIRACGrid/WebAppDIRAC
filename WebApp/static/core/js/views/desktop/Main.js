...
Ext.define("Ext.dirac.views.desktop.Main", {
  ...
  requires: [
    ...
    "Ext.menu.Menu",
    "Ext.view.View",
    "Ext.dirac.views.desktop.Window",
    "Ext.dirac.views.desktop.TaskBar",
  ...
  mixins: [
    "Ext.dirac.core.Stateful",
    "Ext.dirac.core.AppView",
    "Ext.dirac.views.desktop.TransformationData"
  ],
        ...
        if (win.loadedObjectType == "app") {
          ...
          if (win.childWindows.length > 0) {
            for (var i = 0; i < win.childWindows.length; i++) {
              if (
                win.childWindows[i].isChildWindow &&
                win.childWindows[i].type == "help"
              ) {
                var helptext = {
  ...
  loadState: function(oData) {
    ...
    if (me.ID in oData["views"]) {
      ...
      for (
        var i = 0, len = oData["views"]["desktop"]["positions"].length;
        i < len;
        i++
      ) {
        ...
        if ("module" in oData["data"][i]) {
          ...
          me.createWindow(
            oAppStateData.loadedObjectType,
            oAppStateData.name,
            oAppStateData
          );
        }
      ...
      me.setDesktopMatrixCells(
        0,
        me.desktopGranularity[0] - 1,
        0,
        me.desktopGranularity[1] - 1,
        false
      );
  ...
  initComponent: function() {
    ...
    me.bbar = me.taskbar = new Ext.dirac.views.desktop.TaskBar(
      me.taskbarConfig
    );
    ...
    resize: function(oComp, w, h, ow, oh, eOpts) {
      ...
      me.windows.each(function(win) {
        if (win.desktopStickMode) {
          ...
          win.setSize(
            Math.round(me.boxSizeX * win.ic_x),
            Math.round(me.boxSizeY * win.ic_y)
          );
          ...
          win.setPosition(oPos[0], oPos[1]);
          win.setSize(
            Math.round(ratioWidth * win.width),
            Math.round(ratioHeight * win.height)
          );
          win.resumeEvent("resize");
          win.resumeEvent("move");
        }
    ...
    if (GLOBAL.URL_STATE.length != "") {
      ...
      if (parseInt(oParts[0], 10) == 0) {
        ...
        for (var i = 0; i < oApps.length; i++) {
           ...
           */
          if (
            !GLOBAL.APP.isValidApplication(oAppParts[0]) ||
            isNaN(parseInt(oAppParts[2], 10)) ||
            isNaN(parseInt(oAppParts[3], 10)) ||
            isNaN(parseInt(oAppParts[4], 10)) ||
            isNaN(parseInt(oAppParts[5], 10)) ||
            isNaN(parseInt(oAppParts[6], 10))
          ) {
            oValid = false;
            break;
          }
  ...
  createDesktopMenu: function() {
    ...
    var funcAfterSave = function(iCode, sAppName, sStateType, sStateName) {
      if (iCode == 1 && me.currentState != sStateName) {
        ...
        if (me.SM.saveWindow) me.SM.saveWindow.close();
      }
    };
    ...
    if (GLOBAL.STATE_MANAGEMENT_ENABLED)
      ret.items.push(
        ...
        },
        {
          ...
          handler: Ext.bind(
            me.SM.oprSaveAppState,
            me.SM,
            ["application", "desktop", me, funcAfterSave],
            false
          ),
          minWindows: 1,
          scope: me
        ...
        },
        {
          ...
          handler: Ext.bind(
            me.SM.formManageStates,
            me.SM,
            ["desktop", funcAfterRemove],
            false
          ),
          scope: me
        }
  ...
  onWindowMove: function(oWindow, iX, iY, eOpts) {
    ...
    if (oWindow.desktopStickMode) {
      ...
      // header
      if (
        GLOBAL.MOUSE_X >= oWindow.x &&
        GLOBAL.MOUSE_X <= oWindow.x + oWindow.getWidth() &&
        GLOBAL.MOUSE_Y >= oWindow.y &&
        ...
        if (
          oCell[0] + oWindow.ic_x - 1 >= me.desktopGranularity[0] ||
          oCell[1] + oWindow.ic_y - 1 >= me.desktopGranularity[1]
        ) {
          bOK = false;
        } else {
          for (var i = oCell[0]; i <= oCell[0] + oWindow.ic_x - 1; i++) {
            for (var j = oCell[1]; j <= oCell[1] + oWindow.ic_y - 1; j++) {
              if (
                !(
                  i >= oWindow.i_x &&
                  i <= oWindow.i_x + oWindow.ic_x - 1 &&
                  j >= oWindow.i_y &&
                  j <= oWindow.i_y + oWindow.ic_y - 1
                )
              ) {
        ...
        if (bOK) {
          ...
          me.setDesktopMatrixCells(
            oWindow.i_x,
            oWindow.i_x + oWindow.ic_x - 1,
            oWindow.i_y,
            oWindow.i_y + oWindow.ic_y - 1,
            false
          );
          ...
          me.setDesktopMatrixCells(
            oCell[0],
            oCell[0] + oWindow.ic_x - 1,
            oCell[1],
            oCell[1] + oWindow.ic_y - 1,
            true
          );
  ...
  getGridCell: function(iX, iY, bWithFixNearestBorder) {
    ...
    if (bWithFixNearestBorder) {
      ...
      if (oResidualX < iBoxWidth / 2) iXAxis--;

      var oResidualY = iY % iBoxHeight;

      if (oResidualY < iBoxHeight / 2) iYAxis--;
    }
  ...
  setDesktopStickMode: function(oWin) {
    ...
    if (oWin.desktopStickMode) {
      ...
      if (oFindEmptyCell[0] >= 0) {
        ...
        me.setDesktopMatrixCells(
          oWin.i_x,
          oWin.i_x + oWin.ic_x - 1,
          oWin.i_y,
          oWin.i_y + oWin.ic_y - 1,
          true
        );
        ...
        oWin.setSize(
          Math.round(oBox[0] * oDim[0]),
          Math.round(oBox[1] * oDim[1])
        );
        oWin.resumeEvents();
        ...
        GLOBAL.APP.CF.alert(
          "No available space on the desktop can be found !",
          "warning"
        );
  ...
  createWindow: function(loadedObjectType, moduleName, setupData) {
    ...
    if (loadedObjectType == "app") {
      ...
      if (GLOBAL.DEV == 0) {
        ...
        oConfig["paths"][oParts[0] + "." + oParts[1] + ".classes"] =
          "static/" + oParts[0] + "/" + oParts[1] + "/build";
  ...
  __loadDesktopStateData: function(sStateName) {
    ...
    me.loadState(
      GLOBAL.APP.SM.getStateData("application", "desktop", sStateName)
    );
    ...
    if (me.currentState != "") {
      ...
      for (var i = 0; i < me.windows.getCount(); i++) {
        ...
        if (oWin != undefined && oWin != null && !oWin.isChildWindow)
          sNewUrlState +=
            (sNewUrlState == "" ? "" : "^") + oWin.getUrlDescription();
      }
  ...
  getViewMainDimensions: function() {
    ...
    return [
      me.getWidth(),
      me.getHeight() - GLOBAL.APP.MAIN_VIEW.taskbar
        ? GLOBAL.APP.MAIN_VIEW.taskbar.getHeight()
        : 0
    ];
  },
  ...
  cbAfterLoadSharedState: function(iCode, sLink, oDataReceived) {
    ...
    if (oDataItems[0] != "desktop") {
      ...
      for (var i = 0, len = oDataReceived["data"].length; i < len; i++) {
        ...
        var loadedObjectType = !appStateData.loadedObjectType
          ? "app"
          : appStateData.loadedObjectType; // TODO
        // We
        // can
        // remove
        // it
