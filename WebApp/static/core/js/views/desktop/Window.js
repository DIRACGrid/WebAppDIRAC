...
Ext.define("Ext.dirac.views.desktop.Window", {
  ...
  loadMask: null,
  /**
   * @property {Ext.dirac.views.desktop.Main} desktop Reference to the
   *           desktop object
   */
  desktop: null,

  /**
   * @cfg {String} Configuration property that sets the resizable borders of
   *      a window
   */
  ...
  afterShow: function() {
    ...
    if (!me.oneTimeAfterShow) {
      if (me.loadedObjectType == "app") me.setLoadedObject(me.setupData);
      else if (me.loadedObjectType == "link")
  ...
  setLoadedObject: function(setupData) {
    ...
    if (setupData != null) {
      if (
        "desktopStickMode" in setupData &&
        parseInt(setupData["desktopStickMode"], 10) == 1
      ) {
        /*
         ...
         */
        me.desktop.setDesktopMatrixCells(
          me.i_x,
          me.i_x + me.ic_x - 1,
          me.i_y,
          me.i_y + me.ic_y - 1,
        ...
        me.setPosition(
          Math.round(me.i_x * me.desktop.boxSizeX),
          Math.round(me.i_y * me.desktop.boxSizeY)
        );
        me.setSize(
          Math.round(me.ic_x * me.desktop.boxSizeX),
          Math.round(me.ic_y * me.desktop.boxSizeY)
        );
        me.resumeEvents();
        ...
        if ("width" in setupData) {
          if (parseInt(setupData.width, 10) <= 100) {
            me.setWidth(
              parseInt(
                (parseInt(setupData.width, 10) * oDesktopDim[0]) / 100,
                10
              )
            );
          } else {
        ...
        if ("height" in setupData) {
          if (parseInt(setupData.height, 10) <= 100) {
            me.setHeight(
              parseInt(
                (parseInt(setupData.height, 10) * oDesktopDim[1]) / 100,
                10
              )
            );
          } else {
        ...
        if ("data" in setupData) {
          if ("currentState" in setupData) {
            if (setupData.currentState != "") {
              ...
              GLOBAL.APP.SM.oprAddActiveState(
      ...
      if ("headerHidden" in setupData) {
        if (setupData["headerHidden"] == 1) me.getHeader().hide();
      }
    } else {
      // if no setupdata is provided

      if (
        me.loadedObject.launcher.x != null &&
        me.loadedObject.launcher.y != null
      ) {
      ...
      if (
        "width" in me.loadedObject.launcher &&
        me.loadedObject.launcher.width > 0
      ) {
        me.setWidth(me.loadedObject.launcher.width);
      } else {
        me.setWidth(600);
      ...
      if (
        "height" in me.loadedObject.launcher &&
        me.loadedObject.launcher.height > 0
      ) {
        me.setHeight(me.loadedObject.launcher.height);
      } else {
        me.setHeight(400);
    ...
    if (me.currentState == "") {
      ...
      me.taskButton.setText(
        Ext.util.Format.ellipsis(me.loadedObject.launcher.title, 20)
      );
    } else {
      me.setTitle(
        me.loadedObject.launcher.title + " [" + me.currentState + "]"
      );
      me.taskButton.setText(
    ...
    if (me.desktopStickMode) me.taskButton.setIconCls("system_pin_window");
    else me.taskButton.setIconCls(me.loadedObject.launcher.iconCls);
    ...
    if ("maximized" in oData && oData["maximized"]) {
      ...
      if ("width" in setupData) {
        if (parseInt(oData.width, 10) <= 100) {
          me.setWidth(
            parseInt((parseInt(oData.width, 10) * oDesktopDim[0]) / 100, 10)
          );
        } else {
      ...
      if ("height" in setupData) {
        if (parseInt(oData.height, 10) <= 100) {
          me.setHeight(
            parseInt((parseInt(oData.height, 10) * oDesktopDim[1]) / 100, 10)
          );
        } else {
  ...
  setPropertiesWhenLink: function(setupData) {
    ...
    if (setupData.x && setupData.y) me.setPosition(setupData.x, setupData.y);
    else {
      me.setPosition(0, 0);
    }

    if (!setupData.width && !setupData.height) me.maximize();
    else {
      if (setupData.width) me.setWidth(setupData.width);

      if (setupData.height) me.setHeight(setupData.height);
    }
    ...
    if (setupData.zIndex) me.setZIndex(setupData.zIndex);
  },
  ...
  addTools: function() {
    ...
    if (me.loadedObjectType == "app") {
       ...
       */
      var iAppStatesLoaded = GLOBAL.APP.SM.isStateLoaded(
        "application",
        me.appClassName,
        "|"
      ); // OK
      ...
      if (iAppStatesLoaded != -2) {
        if (GLOBAL.STATE_MANAGEMENT_ENABLED) me.oprRefreshAppStates();
      } else {
        /*
        ...
        if (GLOBAL.STATE_MANAGEMENT_ENABLED)
          GLOBAL.APP.SM.oprReadApplicationStatesAndReferences(
            me.appClassName,
            oFunc
          );
      }
      ...
      var funcAfterSave = function(iCode, sAppName, sStateType, sStateName) {
        if (iCode == 1 && me.currentState != sStateName) {
          me.desktop.addStateToExistingWindows(
            "application",
            sStateName,
            sAppName
          );
          ...
          me.setTitle(
            me.loadedObject.launcher.title +
              " [" +
              me.loadedObject.currentState +
              "]"
          );
          me.taskButton.setText(
            Ext.util.Format.ellipsis(
      ...
      me.loadMenu = new Ext.menu.Menu({
        items: [
          ...
          },
          {
            ...
            handler: Ext.bind(
              me.desktop.SM.formManageStates,
              me.desktop.SM,
              [me.loadedObject.self.getName(), funcAfterRemove],
              false
            ),
            scope: me
          }
  ...
  addNewState: function(stateType, stateName) {
    ...
    if (stateType == "application") {
              ...
              GLOBAL.APP.SM.oprShareState(
                ...
                function(rCode, rAppName, rStateName, rMessage) {
                  if (rCode == 1) {
                    var oHtml = "";
                    oHtml +=
                      "<div style='padding:5px'>The string you can send is as follows:</div>";
                    oHtml +=
                    ...
                    Ext.MessageBox.alert(
                      "Info for sharing the <span style='color:red'>" +
                        rStateName +
                        "</span> state:",
                      oHtml
                    );
      ...
      for (var i = me.statesMenu.items.length - 1; i >= 0; i--) {
        if (
          me.statesMenu.items.getAt(i).self.getName() == "Ext.menu.Separator"
        ) {
          iIndexPosition = i;
          break;
        }
      ...
      var newItem = Ext.create("Ext.menu.Item", {
        text: stateName,
        handler: Ext.bind(
          me.desktop.loadSharedStateByName,
          me.desktop,
          [me.appClassName, stateName],
          false
        ),
        scope: me,
  ...
  /**
   * Function to refresh the states of a module. The states are read from
   * the cash.
   */
  oprRefreshAppStates: function() {
    ...
    var oStates = GLOBAL.APP.SM.getApplicationStates(
      "application",
      me.appClassName
    ); // OK
        ...
        menu: [
          {
            text: "Share state",
            handler: function() {
              GLOBAL.APP.SM.oprShareState(me.appClassName, stateName, function(
                rCode,
                rAppName,
                rStateName,
                rMessage
    ...
    var oRefs = GLOBAL.APP.SM.getApplicationStates(
      "reference",
      me.appClassName
    ); // OK
  ...
  oprLoadAppStateFromCache: function(stateName) {
    ...
    var iStateLoaded = GLOBAL.APP.SM.isStateLoaded(
      "application",
      me.appClassName,
      stateName
    ); // OK
    ...
    var statData = GLOBAL.APP.SM.getStateData(
      "application",
      me.appClassName,
      stateName
    );
    ...
    me.taskButton.setText(
      Ext.util.Format.ellipsis(
        me.loadedObject.launcher.title + " [" + stateName + "]",
        20
      )
    );
    me.loadMask.hide();
  ...
  getUrlDescription: function() {
    ...
    var oState = "0";
    if (me.minimized) oState = -1;
    else if (me.maximized) oState = 1;
    ...
    sRet += oState + ":";
    sRet +=
      (me.desktopStickMode ? "1" : "0") +
      "," +
      (me.getHeader().hidden ? "1" : "0") +
      "," +
      me.i_x +
      "," +
      me.i_y +
  ...
  openHelpWindow: function() {
    ...
    me.desktop.createHelpWindow(
      "app",
      "DIRAC.Notepad.classes.Notepad",
      this.loadedObject.getHelpText(),
      win
    );
    win.show();
  }
